...
let updateMergeSplitWireLabels (model: Model) dispatch =
    ...
    let mwStartLabel = SymbolUpdate.generateLabel symModel MergeWires
    let mutable mwIdx =
        (mwStartLabel.[mwStartLabel.Length - 1] |> int)
        - (int '0')

    let swStartLabel = SymbolUpdate.generateLabel symModel (SplitWire 1)
    let mutable swIdx =
        (swStartLabel.[swStartLabel.Length - 1] |> int)
        - (int '0')
...
let getPortIdsfromConnectionId (cid: ConnectionId) (conns: Connection list) =
    ([], conns)
    ||> List.fold (fun pIds c ->
        if c.Id = (string cid) then
            pIds @ [ c.Source.Id; c.Target.Id ]
        else
            pIds)
...
let isPortInConnections (port: Port) (conns: Connection list) =
    (false, conns)
    ||> List.fold (fun b c ->
        (port.Id = c.Source.Id || port.Id = c.Target.Id)
        || b)
...
let addInternalConnections
    ...
    let addedConns = wholeCanvasConns |> List.filter shouldAdd
...
let correctCanvasState (selectedCanvasState: CanvasState) (wholeCanvasState: CanvasState) =
    let components, connections =
        addInternalConnections selectedCanvasState wholeCanvasState
    ...
    and getPortWidthFromComponent port pw run =
        let hostComponent =
            components
        ...
        | Custom c ->
            let indexInput =
                List.tryFindIndex (fun (p: Port) -> p.Id = port.Id) hostComponent.InputPorts
            let indexOutput =
                List.tryFindIndex (fun (p: Port) -> p.Id = port.Id) hostComponent.OutputPorts
            match (indexInput, indexOutput) with
            | (Some i, None) -> c.InputLabels[i] |> snd |> Some
            | (None, Some o) -> c.OutputLabels[o] |> snd |> Some
            | _ -> None
        //let s = List.tryFind ()
        | _ -> None
    ...
    let getPortWidth (port: Port) =
        ...
        | Error e -> Error e
        | Ok pw -> Ok <| getPortWidth' port pw 0
    ...
    let inferIOLabel (port: Port) =
        ...
        | None, _ ->
            failwithf "what? no PortNumber. A connection port was probably passed to inferIOLabel"
        | Some pn, PortType.Input ->
            match Symbol.portNames hostComponent.Type with
            | ([]) when List.length hostComponent.InputPorts > 1 ->
                hostComponent.Label + "_IN" + (string pn)
            | ([]) -> hostComponent.Label + ".IN"
            | lst ->
                if pn >= lst.Length then
                    failwithf
                        "what? input PortNumber is greater than number of input port names on component"
                else
                    lst[pn] //hostComponent.Label + "." + lst[pn]
        | Some pn, PortType.Output ->
            match Symbol.portNames hostComponent.Type with
            | ([]) when List.length hostComponent.OutputPorts > 1 ->
                hostComponent.Label + "_OUT" + (string pn)
            | ([]) -> hostComponent.Label + "_OUT"
            | lst ->
                if pn >= lst.Length then
                    failwithf
                        "what? output PortNumber is greater than number of output port names on component"
                else
    ...
    let addExtraIOs (comps: Component list, conns: Connection list) =
        let compsOk: Result<Component, SimulationError> list =
            List.map (fun c -> Ok c) comps
        let result =
            (compsOk, conns)
            ||> List.mapFold (fun acc con ->
                ...
                if
                    not (isPortInComponents con.Source comps)
                    && not (isPortInComponents con.Target comps)
                then
                    let error =
                    ...
                    | Ok(Some pw) ->
                        ...
                        let extraInput =
                            { Id = newId
                              ...
                              W = 0
                              SymbolInfo = None }
                        Ok { con with Source = { newPort with PortNumber = None } },
                        acc @ [ Ok extraInput ]
                    | Ok(None) ->
                        let error =
                            { Msg =
                                "Could not infer the width for an input into the selected logic."
                              InDependency = None
                    ...
                    | Ok(Some pw) ->
                        ...
                        let extraOutput =
                            { Id = newId
                              ...
                              W = 0
                              SymbolInfo = None }
                        Ok { con with Target = { newPort with PortNumber = None } },
                        acc @ [ Ok extraOutput ]
                    | Ok(None) ->
                        let error =
                            { Msg =
                                "Could not infer the width for an output produced by the selected logic."
                              InDependency = None
    ...
    let checkCanvasWasCorrected
        (
            compsRes: Result<Component, SimulationError> list,
            connsRes: Result<Connection, SimulationError> list
        )
        =
...
let makeSimDataSelected
    (model: Model)
    : (Result<SimulationData, SimulationError> * CanvasState) option
    =
    ...
    | [], _, _ ->
        ...
        <| (Error
                { Msg =
                    "Only connections selected. Please select a combination of connections and components."
                  InDependency = None
                  ComponentsAffected = []
                  ConnectionsAffected = affected },
            (selComponents, selConnections))
    | selComps, selConns, Some project ->
        ...
        if stateIsEqual state selCache.UncorrectedCanvas then
            ...
            | Ok(correctComps, correctConns) ->
                match
                    CanvasStateAnalyser.analyseState
    ...
    let leftArrow =
        Button.button
            [ Button.Props [ colMoveArrowStyle ]
              Button.OnClick(fun _ -> (io, MLeft) |> MoveColumn |> dispatch) ]
            [ str "<" ]
    let rightArrow =
        Button.button
            [ Button.Props [ colMoveArrowStyle ]
              Button.OnClick(fun _ -> (io, MRight) |> MoveColumn |> dispatch) ]
            [ str ">" ]
    div [] [ makeElementLine [ leftArrow ] [ rightArrow ]; headingEl ]

let private makeMenuGroup openDefault title menuList =
    details [ Open openDefault ] [ summary [ menuLabelStyle ] [ str title ]; Menu.list [] menuList ]

let viewOutputHider table hidden dispatch =
    let makeToggleRow io =
        let isChecked = not <| List.contains io hidden
        let changeAction = (fun _ -> dispatch <| ToggleHideTTColumn io)
...
let viewCellAsHeading
    dispatch
    sortInfo
    (styleInfo: Map<CellIO, CSSProp list>)
    (cell: TruthTableCell)
    =
    let addMoveArrows el =
        makeColumnMoveArrows cell.IO el dispatch
    let cellStyle =
        ...
        | Some s ->
            Style
            <| (FontWeight "bold")
               :: (s @ [ BorderBottom "3px solid black" ])
    match cell.IO with
    | SimIO(_, label, _) ->
        ...
        div
            [ cellStyle ]
            [ makeElementLine
                  [ (str headingText) ] //[makeColumnMoveArrows cell.IO (str headingText) dispatch]
    ...
    | Viewer((label, fullName), width) ->
        let headingEl =
            ...
            |> (fun r ->
                if fullName <> "" then
                    addToolTipTop fullName r
                else
                    r)
        div
            [ cellStyle ]
            [ makeElementLine
                  [ headingEl ] //[makeColumnMoveArrows cell.IO headingEl dispatch]
                  [ makeSortingArrows cell.IO sortInfo dispatch ]
              |> addMoveArrows ]
...
let viewRowAsData numBase styleInfo i (row: TruthTableCell list) =
    ...
    let cells = row |> List.map viewCellAsData
    cells

let viewTruthTableError simError =
    let error =
        match simError.InDependency with
        | None -> div [] [ str simError.Msg; br []; str <| "Please fix the error and retry." ]
        | Some dep ->
            div
                []
                [ str
                  <| "Error found in dependency \"" + dep + "\":"
                  br []
                  str simError.Msg
                  br []
                  str
                  <| "Please fix the error in the dependency and retry." ]
    div
        []
...
let viewTruthTable canvasState model dispatch =
    ...
    | None ->
        let wholeSimRes = SimulationView.simulateModelFData None 2 canvasState model
        let wholeButton =
    ...
    | Some tableopt ->
            ...
            | Ok table ->
                let truncation =
                    Notification.notification
                        [ Notification.Color IsWarning; Notification.IsLight ]
                        [ str
                              "Due to a large number of input combinations, caused by inputs that are
        ...
        div
            ...
            [ Button.button
              ...
              br []
              str
                  "The Truth Table generator uses the diagram as it was at the moment of
