...
let rotateLabel (sym: Symbol) =
    ...
    { sym with LabelRotation = newRot; LabelHasDefaultPos = true }
...
let rotateSelectedLabelsClockwise (model: Model) =
    ...
    (symMap, syms)
    ||> List.fold (fun sMap sym -> Map.add sym.Id ((rotateLabel >> Symbol.calcLabelBoundingBox) sym) sMap)
    |> (fun sMap -> Optic.set symbols_ sMap model, Cmd.none)
...
let workOutArrangement (arrange: Arrange) (syms: Symbol list) =
    ...
    |> Option.map (fun (syms, bbData) ->
        ...
        | [], _, _ -> [], Error "No alignable symbols found"
        ...
        | syms, Some(orient, _), _ -> syms, Ok orient
        | syms, _, _ -> syms, Error "alignment failed")
...
let distributePosition (symbols: Symbol list) (isX: bool) =
    ...
    |> (fun lst ->
        ...
        |> List.mapi (fun i (f, sym) ->
            ...
            [ Symbol <| MoveSymbols([ sym.Id ], offset); BusWireT.UpdateSymbolWires sym.Id ])
        |> List.concat)
...
let arrangeSymbols (arrange: Arrange) (model: Model) : Model * Cmd<ModelType.Msg> =
    ...
    let newSelected =
        syms
        |> List.map (fun sym -> ComponentId sym.Component.Id)
    match result with
    | Error _mess -> { model with SelectedComponents = newSelected }, Cmd.none
    | Ok orientation ->
        let postludeCmds = [ sheetCmd UpdateBoundingBoxes ]
        let cmds =
            match arrange with
            | AlignSymbols -> alignPosition syms (orientation = Vertical)
...
let moveSymbols (model: Model) (mMsg: MouseT) =
    ...
    | [ symId ] -> // Attempt Snap-to-Grid if there is only one moving component
        let symbol =
            ...
            | Some symbol -> symbol
            | None -> failwithf "What? can't move a symbol which does not exist in the model!"
        ...
        let errorComponents =
            if notIntersectingComponents model bBox compId then
                []
            else
                [ compId ]
        { model with
        ...
        Cmd.batch
            [ symbolCmd (MoveSymbols(model.SelectedComponents, moveDelta))
    ...
    | _ -> // Moving multiple symbols -> don't do snap-to-grid
        ...
        { model with
            ...
            ErrorComponents = errorComponents },
        Cmd.batch
            [ symbolCmd (SymbolT.MoveSymbols(model.SelectedComponents, mMsg.Pos - model.LastMousePos))
...
let snapWire (model: Model) (mMsg: MouseT) (segIdL: SegmentId list) : Model * Cmd<ModelType.Msg> =
    ...
    | segId :: _ ->
        ...
        { model with
            ...
            SnapSegments = snapXY },
        Cmd.batch
            [ wireCmd (BusWireT.DragSegment(segIdL, newmMsg))
              sheetCmd CheckAutomaticScrolling ]
...
let mDownUpdate (model: Model) (mMsg: MouseT) : Model * Cmd<ModelType.Msg> =
    let newModel =
    ...
    | _ ->
        ...
        | InputPort(portId, portLoc) ->
            if not model.CtrlKeyDown then
                { model with
                    Action = ConnectingInput portId
                    ConnectPortsLine = portLoc, mMsg.Pos
                    TmpModel = Some model },
                ...
                let portIdstr =
                    match portId with
                    | InputPortId x -> x
                { model with Action = MovingPort portIdstr }, symbolCmd (SymbolT.MovePort(portIdstr, mMsg.Pos))

        | OutputPort(portId, portLoc) ->
            if not model.CtrlKeyDown then
                { model with
                    Action = ConnectingOutput portId
                    ConnectPortsLine = portLoc, mMsg.Pos
                    TmpModel = Some model },
        ...
        | ComponentCorner(compId, fixedCornerLoc, _) ->
            if not model.CtrlKeyDown then
                ...
                { model with
                    Action = ResizingSymbol(compId, fixedCornerLoc)
                    LastValidSymbol = Some symbol },
        ...
        | Component compId ->
            ...
            | Some(RotateButton rotation) ->
                ...
                Cmd.batch
                    [ sheetCmd (Rotate rotation)
                      wireCmd (BusWireT.UpdateConnectedWires model.SelectedComponents) ]

            | _ ->
                let msg, action = DoNothing, InitialiseMoving compId
                if model.CtrlKeyDown || mMsg.ShiftKeyDown then
                    let newComponents =
                        if List.contains compId model.SelectedComponents then
                            List.filter (fun cId -> cId <> compId) model.SelectedComponents // If component selected was already in the list, remove it
                        else
                            compId :: model.SelectedComponents // If user clicked on a new component add it to the selected list
                    ...
                    let newComponents, newWires =
                        if List.contains compId model.SelectedComponents then
                            model.SelectedComponents, model.SelectedWires // Keep selection for symbol movement
                        else
                            [ compId ], [] // If user clicked on a new component, select that one instead
                    let snapXY =
                        match newComponents with
                        | [ compId ] -> getNewSymbolSnapInfo model model.Wire.Symbol.Symbols[compId]
                        | _ -> emptySnap
                    { model with
                        SelectedComponents = newComponents
                        SnapSymbols = snapXY
                    ...
                    Cmd.batch
                        [ symbolCmd (SymbolT.SelectSymbols newComponents)
        ...
        | Connection connId ->
            ...
            if model.CtrlKeyDown then
                let newWires =
                    if List.contains connId model.SelectedWires then
                        List.filter (fun cId -> cId <> connId) model.SelectedWires // If component selected was already in the list, remove it
                    else
                        connId :: model.SelectedWires // If user clicked on a new component add it to the selected list
                ...
                | _ ->
                    ...
                    { model with Action = DragAndDrop }, Cmd.batch [ sheetCmd DoNothing ]
            else
        ...
        | Canvas ->
            let newComponents, newWires =
                if model.CtrlKeyDown then
                    model.SelectedComponents, model.SelectedWires //do not deselect if in CtrlKeyDown mode
                else
                    [], []
            ...
            | true ->
                ...
                | _ ->
                    printfn "Error components (Right)"
                    { model with Action = DragAndDrop }, Cmd.none
...
let mDragUpdate (model: Model) (mMsg: MouseT) : Model * Cmd<ModelType.Msg> =
    ...
    match model.Action with
    | MovingWire segIdL -> snapWire model mMsg segIdL
    // HLP 23: AUTHOR Khoury & Ismagilov
    ...
    | Scaling ->
        ...
        let newScalingBoxOppositeMouse =
            { X =
                scalingBoxCentre.X
                - (mMsg.Pos.X - scalingBoxCentre.X)
              Y =
                scalingBoxCentre.Y
                - (mMsg.Pos.Y - scalingBoxCentre.Y) }
        ...
        let newBBMin =
            { X =
                min (newScalingBoxOppositeMouse.X) (mMsg.Pos.X)
                + 50.
              Y =
                min (newScalingBoxOppositeMouse.Y) (mMsg.Pos.Y)
                + 50. }
        let newBBMax =
            { X =
                max (newScalingBoxOppositeMouse.X) (mMsg.Pos.X)
                - 50.
              Y =
                max (newScalingBoxOppositeMouse.Y) (mMsg.Pos.Y)
                - 50. }

        let selectedSymbols =
            RotateScale.findSelectedSymbols (modelBeforeUpdate.SelectedComponents) (modelBeforeUpdate.Wire.Symbol)

        let xYSC =
            RotateScale.getScalingFactorAndOffsetCentreGroup newBBMin newBBMax selectedSymbols
        let scaleSymFunc = RotateScale.scaleSymbol xYSC
        let newSymModel =
            RotateScale.groupNewSelectedSymsModel
                (modelBeforeUpdate.SelectedComponents)
                (modelBeforeUpdate.Wire.Symbol)
                selectedSymbols
                scaleSymFunc
        let newModel =
            { { model with Wire = { model.Wire with Symbol = newSymModel } } with
                BoundingBoxes =
                    Symbol.getBoundingBoxes { model with Wire = { model.Wire with Symbol = newSymModel } }.Wire.Symbol }

        let newSelectedSymbols =
            RotateScale.findSelectedSymbols (newModel.SelectedComponents) (newModel.Wire.Symbol)
        ...
        let staySameModel =
            if errorSelectedComponents <> [] then
                (Some modelBeforeUpdate)
            elif
                (oneCompBoundsBothEdges
                 && model.ScalingTmpModel.IsSome)
            then
                (modelBeforeUpdate.ScalingTmpModel)
            else
                None
    ...
    | Selecting ->
        ...
        let newDragToSelectBox =
            { model.DragToSelectBox with
                W = (mMsg.Pos.X - initialX)
                H = (mMsg.Pos.Y - initialY) }
        { model with
            DragToSelectBox = newDragToSelectBox
            ScrollingLastMousePos = { Pos = mMsg.Pos; Move = mMsg.ScreenMovement }
            LastMousePos = mMsg.Pos },
        sheetCmd CheckAutomaticScrolling
    ...
    | MovingLabel ->
        let movingCompId =
            ...
            | Some compid -> compid
            | None -> failwithf "What? no component found for moving label operation"
        { model with
            Action = MovingLabel
            CursorType = Grabbing
            LastMousePos = mMsg.Pos
    ...
    | MovingPort portId -> model, symbolCmd (SymbolT.MovePort(portId, mMsg.Pos))
    // HLP23 AUTHOR: BRYAN TAN
...
let mUpUpdate (model: Model) (mMsg: MouseT) : Model * Cmd<ModelType.Msg> = // mMsg is currently un-used, but kept for future possibilities
    ...
    | MovingWire segIdL ->
        ...
        let coalesceCmds =
            connIdL
            |> List.map (fun conn -> wireCmd (BusWireT.CoalesceWire conn))
        { model with Action = Idle; UndoList = appendUndoList model.UndoList newModel },
        Cmd.batch (
            [ wireCmd (BusWireT.DragSegment(segIdL, mMsg))
              wireCmd (BusWireT.MakeJumps(true, connIdL)) ]
            @ coalesceCmds
        )
    | Selecting ->
        ...
        { model with
            ...
            Action = Idle
            SelectedComponents = selectComps
            SelectedWires = selectWires
            AutomaticScrolling = false },
        Cmd.batch
            [ symbolCmd (SymbolT.SelectSymbols selectComps)
              sheetCmd DoNothing
    ...
    | InitialiseMovingLabel compId -> { model with Action = Idle; SelectedLabel = Some compId }, sheetCmd DoNothing

    | MovingLabel -> { model with Action = Idle; UndoList = appendUndoList model.UndoList newModel }, sheetCmd DoNothing
    ...
    | MovingSymbols ->
        ...
        | [] ->
            let movingWires =
                BusWireUpdateHelpers.getConnectedWireIds model.Wire model.SelectedComponents
            ...
            | _ ->
                { model with
                    ...
                    AutomaticScrolling = false },
                Cmd.batch
                    [ //symbolCmd (SymbolT.MoveSymbols (model.SelectedComponents, (model.LastValidPos - mMsg.Pos)))
                      symbolCmd (SymbolT.SelectSymbols(model.SelectedComponents))
    ...
    | ConnectingInput inputPortId ->
        let cmd, undoList, redoList =
            if
                model.TargetPortId <> "" // If a target has been found, connect a wire\
            then
                wireCmd (BusWireT.AddWire(inputPortId, (OutputPortId model.TargetPortId))),
                appendUndoList model.UndoList newModel,
                newModel.RedoList
            else
                Cmd.none, newModel.UndoList, newModel.RedoList
        { model with
            Action = Idle
            TargetPortId = ""
            UndoList = undoList
    ...
    | ConnectingOutput outputPortId ->
        let cmd, undoList, redoList =
            if
                model.TargetPortId <> "" // If a target has been found, connect a wire
            then
                wireCmd (BusWireT.AddWire(InputPortId model.TargetPortId, outputPortId)),
                appendUndoList model.UndoList newModel,
                newModel.RedoList
            else
                Cmd.none, newModel.UndoList, newModel.RedoList
...
let mMoveUpdate (model: Model) (mMsg: MouseT) : Model * Cmd<ModelType.Msg> =
    match model.Action with
    | DragAndDrop -> moveSymbols model mMsg
    ...
    | InitialisedCreateComponent(ldcs, compType, lbl) ->
        let labelTest =
            ...
            | Output _
            | Viewer _
            | IOLabel -> SymbolUpdate.generateIOLabel model.Wire.Symbol compType lbl
            | _ ->
                if lbl = "" then
                    SymbolUpdate.generateLabel model.Wire.Symbol compType
                else
                    lbl
        let newSymbolModel, newCompId =
            SymbolUpdate.addSymbol ldcs model.Wire.Symbol mMsg.Pos compType labelTest

        { model with
            Wire = { model.Wire with Symbol = newSymbolModel }
            Action = DragAndDrop
    ...
    | _ ->
        ...
        // HLP23 AUTHOR: BRYAN TAN
        let ctrlPressed =
            List.exists (fun (k, _) -> k = "CONTROL") (SheetDisplay.getActivePressedKeys model)
        let newCursor =
            ...
            | _ ->
                ...
                | InputPort(_, p)
                | OutputPort(_, p) -> ClickablePort // Change cursor if on port
        ...
        if ctrlPressed then
            newModel,
            Cmd.batch
                [ symbolCmd (SymbolT.ShowCustomOnlyPorts nearbyComponents)
                  symbolCmd (SymbolT.ShowCustomCorners nearbyComponents) ]
        else
...
let getVisibleScreenCentre (model: Model) : XYPos =
    ...
    { X =
        (canvas.scrollLeft + canvas.clientWidth / 2.0)
        / model.Zoom
      Y =
        (canvas.scrollTop + canvas.clientHeight / 2.0)
        / model.Zoom }
...
let validateTwoSelectedSymbols (model: Model) =
    ...
    | [ s1; s2 ] as syms ->
        ...
        let getSym sId = Map.tryFind sId symbols
...
let rec getChannel (bb1: BoundingBox) (bb2: BoundingBox) : (BoundingBox * Orientation) option =
    if bb1.TopLeft.X > bb2.TopLeft.X then
        getChannel bb2 bb1
    else if overlap2DBox bb1 bb2 then
        None // boxes intersect, invalid channel
    elif
        (bb1.TopLeft.Y > bb2.TopLeft.Y + bb2.H
         || bb1.TopLeft.Y + bb1.H < bb2.TopLeft.Y)
        && bb2.TopLeft.X > bb1.TopLeft.X + bb1.W
    then
