...
/// The id a component gets in the generated canvas: one more than its position in the
...
let toCanvasState (sheet: SheetDescription) : Result<CanvasState, string> =
    ...
    | [] ->
        sheet.Comps
        |> List.mapi (fun i spec -> i + 1, spec)
        |> Helpers.ResultList.traverse (fun (id, spec) -> buildComponent id spec)
