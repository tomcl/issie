...
module Constants =
    ...
    let outOfDateMessage =
        "Use refresh button to update waveforms. 'End' and then 'Start' to simulate a different sheet"
...
let listMaxWithDef defaultValue lst = defaultValue :: lst |> List.max
...
let highZoom clkCycleWidth =
    clkCycleWidth < 2. * Constants.nonBinaryTransLen
...
let rec getWSModel model : WaveSimModel =
    ...
    | None ->
        ...
        | None -> initWSModel
        | Some sheet -> getWSModel { model with WaveSimSheet = Some sheet }

/// Width of one clock cycle.
let singleWaveWidth m =
    max 5.0 (float m.WaveformColumnWidth / float m.ShownCycles)

/// Left-most coordinate of the SVG viewbox.
let viewBoxMinX m =
    string (float m.StartCycle * singleWaveWidth m)

/// Total width of the SVG viewbox.
let viewBoxWidth m =
    string (max 5.0 (m.WaveformColumnWidth))

/// Right-most visible clock cycle.
let endCycle wsModel =
    wsModel.StartCycle + (wsModel.ShownCycles) - 1

/// Helper function to create Bulma buttons
let button options func label =
    Button.button (List.append options [ Button.OnClick func ]) [ label ]
...
let pointsToString (points: XYPos array) : string =
    Array.fold (fun str (point: XYPos) -> $"{str} %.1f{point.X},%.1f{point.Y} ") "" points
...
let getWaveValue (currClkCycle: int) (wave: Wave) (width: int) : FastData =
    ...
    |> function
        ...
        | _ ->
            ...
            printf
                "Trying to access index %A in wave %A. Default to 0."
                currClkCycle
                wave.DisplayName
...
let makeXCoords (clkCycleWidth: float) (clkCycle: int) (transition: Transition) =
    ...
    | BinaryTransition _ -> float clkCycle * clkCycleWidth, float (clkCycle + 1) * clkCycleWidth
    | NonBinaryTransition _ ->
        ...
        float clkCycle * clkCycleWidth
        - xShift clkCycleWidth,
        float (clkCycle + 1) * clkCycleWidth
        - xShift clkCycleWidth
...
let binaryWavePoints
    ...
    let topL, topR, botL, botR =
        makeCoords clkCycleWidth (startCycle + index) (BinaryTransition transition)
...
let nonBinaryWavePoints
    (clkCycleWidth: float)
    (startCycle: int)
    (index: int)
    (transition: NonBinaryTransition)
    : (XYPos array * XYPos array)
    =
    let xLeft, _ =
        makeXCoords clkCycleWidth (startCycle + index) (NonBinaryTransition transition)
    let _, topR, _, botR =
        makeCoords clkCycleWidth (startCycle + index) (NonBinaryTransition transition)
...
let isWaveSelected (wsModel: WaveSimModel) (index: WaveIndexT) : bool =
    List.contains index wsModel.SelectedWaves

let isRamSelected (ramId: FComponentId) (wsModel: WaveSimModel) : bool =
...
let getCompDetails fs wave =
    ...
    let descr, oneLine =
        ...
        | NbitsAdderNoCin n
        | NbitsAdderNoCout n
        | NbitsAdderNoCinCout n -> $"{n} bit adder", false
        ...
        | AsyncROM1 mem ->
            $"ROM  ({1 <<< mem.AddressWidth} word X {mem.WordWidth} bit) asynchronous read", false
        | ROM1 mem ->
            $"ROM  ({1 <<< mem.AddressWidth} word X {mem.WordWidth} bit) synchronous read", false
        | RAM1 mem ->
            $"RAM  ({1 <<< mem.AddressWidth} word X {mem.WordWidth} bit) synchronous read", false
        | AsyncRAM1 mem ->
            $"RAM  ({1 <<< mem.AddressWidth} word X {mem.WordWidth} bit) asynchronous read", false
        | BusSelection _
        | MergeWires
        | SplitWire _ -> failwithf "Bus select, MergeWires, SplitWire should not appear"
        | Input _
        | Constant _
        | AsyncROM _
        | ROM _
        | RAM _ -> failwithf "Legacy component types should not appear"
        | Shift _ -> "Error: Shift is an internal component that should not appear", false
    match oneLine with
...
let getCompGroup fs wave =
    ...
    | CounterNoEnable _
    | CounterNoLoad _
    | CounterNoEnableLoad _ -> FFRegister
    | AsyncROM1 _
    | ROM1 _
    | RAM1 _
    | AsyncRAM1 _ -> Memories
    | BusSelection _
    | MergeWires
    | SplitWire _ -> failwithf "Bus select, MergeWires, SplitWire should not appear"
    | Input _
...
let summaryName
    ...
    =
    match cBox with
    | PortItem(_, name) -> str <| camelCaseDottedWords name
    | ComponentItem fc ->
    ...
    | SheetItem subSheet ->
        str
        <| $"Subsheet {camelCaseDottedWords subSheet[subSheet.Length - 1]}"

let path2fId (fastSim: FastSimulation) (path: ComponentId list) : FComponentId option =
    match path with
    | [] -> None
    | p -> Some <| (p[p.Length - 1], p[0 .. p.Length - 2])
...
let sheetIdToName (fastSim: FastSimulation) sheetId =
    ...
    |> function
        | None -> []
        | Some fId -> [ fastSim.WaveComps[fId].FLabel ]
...
let sheetIdToSubsheets (fastSim: FastSimulation) (sheetId: ComponentId list) =
    ...
    | n ->
        [ 1..n ]
        |> List.collect (fun i -> sheetId[0 .. i - 1] |> sheetIdToName fastSim)
...
let prefixOf (pre: 'a list) (whole: 'a list) =
    whole.Length >= pre.Length
    && whole[0 .. pre.Length - 1] = pre
...
let getFastSimualationLinkedPorts (fs: FastSimulation) ((fc, pNum, pType)) = fs.FSComps

/// Some components have input and output connected - return both ports in that case
let getConnectedComponentPorts
    (ws: WaveSimModel)
    ((fc, portNum, portType) as port: PortIndex)
    : PortIndex list
    =
    let fs = ws.FastSim
    ...
    match fc.FType with
    | Output _ -> portIO
    | IOLabel when Map.containsKey (ComponentLabel fc.FLabel, snd fc.fId) fs.FIOActive -> portIO
    | _ -> [ port ]
...
let waveToSheetPort fs (wave: Wave) =
    ...
    |> Option.map (fun comp ->
        let port =
            ...
            | PortType.Input, true, _
            | PortType.Output, true, false -> comp.InputPorts[wi.PortNumber]
            | PortType.Output, _, true
            | PortType.Input, false, true -> comp.OutputPorts[wi.PortNumber]
            | _ -> failwithf "What? no parts found in waveToSheetPort"
        { Sheet = sheet.ToLower(); PortOnComp = port }
        |> fun p -> [ p ])
    |> Option.defaultValue []
...
let connectedIOs (fs: FastSimulation) (sp: SheetPort) =
    ...
    | { Type = IOLabel } as comp ->
        ...
        |> Seq.toList
        |> List.collect (function
            | { Type = IOLabel; Label = label } as comp1 when label = comp.Label ->
                ((if comp1.OutputPorts.Length > 0 then
                      [ { Sheet = sheet; PortOnComp = comp1.OutputPorts[0] } ]
                  else
                      [])
                 @ (if comp1.InputPorts.Length > 0 then
                        [ { Sheet = sheet; PortOnComp = comp1.InputPorts[0] } ]
                    else
                        [])
                 @ [ sp ])
            | _ -> [])
...
let connsOfWave (fs: FastSimulation) (wave: Wave) =
    ...
    |> List.collect (fun sp ->
        match Map.tryFind sp fs.ConnectionsByPort with
        | None -> []
        | Some conns -> conns)
...
let infoButton
    ...
    div
        [ HTMLAttr.ClassName
              $"{Tooltip.ClassName} {Tooltip.IsMultiline} {Tooltip.IsInfo} {tooltipPosition}"
...
let waveInfoButton (dispatch: Msg -> Unit) : ReactElement =
    button
        [ Button.Props
              [ Style
                    [ FontSize "25px"; MarginTop "0px"; MarginLeft "10px"; Float FloatOptions.Left ] ] ]
...
let removeHighlights (model: Model) dispatch =
    if
        model.Sheet.SelectedWires.Length > 0
        || model.Sheet.SelectedComponents.Length > 0
    then
        dispatch
        <| Sheet(DrawModelType.SheetT.ResetSelection) // Remove highlights.
...
let getWaveSimButtonOptions
    ...
    let fs = ws.FastSim
    let simExists =
        model.WaveSimSheet <> Some ""
        && model.WaveSimSheet <> None
    let errored =
        match ws.State with
        | SimError _
        | NonSequential -> true
    ...
    let isDirty =
        ...
        && running
        && not
           <| FastRun.compareLoadedStates fs canv model.CurrentProj
        && model.UIState = None
        && not model.IsLoading
