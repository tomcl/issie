...
let dec2hex (n: bigint) (nBits: uint32) : string =
    ...
    let paddedBin = dec2bin n nBits |> Seq.append seqPad |> Seq.toList
    ...
    let fourBit2HexDig =
        ...
        [ 0..15 ]
        |> List.map (fun dig -> List.map (bit dig) [ 3..-1..0 ], hexDigOf dig)
        |> Map.ofList
...
let dec2sdec (n: bigint) (nBits: uint32) =
    if (dec2bin n nBits)[0] = '1' then
        n - bigint (2.0 ** (float nBits))
    else
        n
    |> string
...
let n2StringOfRadix (hasRadixPrefix: bool) (n: bigint) (nBits: uint32) (rad: NumberBase) =
    ...
    |> (fun numberRepStr ->
        (if hasRadixPrefix then pref else "")
        + numberRepStr)
...
let reactTickBoxRow name nameStyle ticked toggleFun =
    tr
        [ Class "rowHeight"; Style [ VerticalAlign "middle" ] ]
        [ td
              [ Class "wAcheckboxCol"; Class "rowHeight"; Style [ VerticalAlign "middle" ] ]
              [ input
                    [ Type "checkbox"
                      Class "check"
                      Checked <| ticked
...
let getWaveSetup (ws: WaveSimModel) (model: Model) : MoreWaveSetup =
    ...
    let rec subSheets
        ((sg, name, label, path): SimulationGraph * string * ComponentLabel * ComponentId list)
        : (SimulationGraph * string * ComponentLabel * ComponentId list) list
        =
        Map.toList sg
        |> List.collect (fun (cId, comp) ->
            match comp.Type with
            | Custom custComp ->
                [ Option.get comp.CustomSimulationGraph, custComp.Name, comp.Label, path @ [ cId ] ]
            | _ -> [])
    ...
    let rec allSheets
        ...
        | sheets ->
            [ sg, name, label, path ]
            @ List.collect allSheets sheets
    let mainSheet =
        ((Option.get ws.InitWaveSimGraph).Graph, model.WaveSimSheet, ComponentLabel "", [])
    ...
    let sheetCol =
        fun (sg, name, ComponentLabel label, path) ->
            ...
            |> List.collect (function
                | (Some(i, ctyp), path) ->
                    [ { Label = label; Sheet = name; Path = path; CSort = ctyp } ]
                | _ -> [])
    sheets
    ...
    let makeWaveReactList (swL: SheetWave list) =
        ...
        |> List.map (fun sw ->
            ...
            let toggle _ =
                let ticks =
                    if ticked then
                        Set.remove sw.Path ticks
                    else
                        Set.add sw.Path ticks
        ...
        |> (fun els ->
            ...
            let uniques =
                ...
                |> Set
            List.filter
                (fun (name, ticked, comp, toggle) -> isRamOrRom comp && Set.contains name uniques)
                els)
            ...
            table
                [ Style
                      [ Display DisplayOptions.InlineBlock
                        ...
                        PaddingLeft "5px"
                        PaddingRight "5px" ] ]
                [ tbody [ Style [ Display DisplayOptions.Inline ] ]
                  <| [ tr
                           []
                           [ th
                                 [ ColSpan 2; Style [ TextAlign TextAlignOptions.Center ] ]
                                 [ str name ] ] ]
                     @ colBody ]
        else
            div [] []
...
let formatMemory (mem: Memory1) =
    ...
    let rec fillGaps curAddr lst =
        ...
        | (addr, dat) :: lst' ->
            dispGap addr curAddr
            @ [ makeRamLoc addr dat ]
              @ fillGaps (addr + 1L) lst'
...
let getRamInfoToDisplay wSMod (path: ComponentId list) =
    ...
    let apLst =
        ...
        |> List.rev
        |> function
            | (h :: rest) -> h :: (List.rev rest)
            | [] -> []
    match sdOpt, apLst with
    | Some sd, (cid :: ap) ->
        let state = FastRun.extractFastSimulationState sd.FastSim (int cursorStep) (cid, ap)
        let lab =
            match sd.FastSim.FComps[cid, ap].SimComponent.Label with
            | ComponentLabel lab -> lab
        match state with
        | RamState dat -> lab, formatMemory dat
        | _ -> lab, []
    | _ -> "", []
...
let private clkAdvance (sD: SimulationData) =
    let sD =
        if sD.ClockTickNumber = 0 then
            ...
            { sD with
                FastSim =
                    match FastRun.buildFastSimulation (int maxLastClk) sD.Graph with
                    | Ok fs -> fs
                    | Error e -> failwithf "fast simulation error" }
        else
            sD
    let newClock = sD.ClockTickNumber + 1
    FastRun.runFastSimulation newClock sD.FastSim
    { sD with Graph = sD.Graph; ClockTickNumber = newClock }
...
let private drivingOutput (netList: NetList) compId inPortN = netList[compId].Inputs[inPortN]
...
let getSimTime (waves: WaveformSpec array) (simData: SimulationData) =
    ...
    |> Array.map (fun wave ->
        try
            ...
            | IData wd -> Wire { NBits = uint (List.length wd); BitData = simWireData2Wire wd }
...
let private nlTrgtLst2CommonNLSource
    (netList: NetList)
    (nlTrgtLst: NLTarget list)
    : NLSource option
    =
    List.tryPick
        (fun (nlTrgt: NLTarget) ->
...
let net2outputsAndIOLabels (netList: NetList) (netLst: NLTarget list) =
    let nlTrgt2Lbls st nlTrgt =
        ...
        | Some nlComp ->
            ...
            | IOLabel
            | Output _ -> List.append st [ netList[nlTrgt.TargetCompId].Label ]
            | _ -> st
        | None -> st
    List.fold nlTrgt2Lbls [] netLst |> List.distinct
...
let private simplifyName name = name |> removeSubSeq '(' ')'

/// get WaveLabel corresponding to a NLTarget list
let rec private findName
    ...
    | Some nlSource ->
        ...
        if not (Set.contains nlSource.SourceCompId compIds) then
            ...
            | Mux4
            | Mux8
            | BusCompare _ -> [ { LabName = compLbl; BitLimits = 0, 0 } ]
            | Input w
            | Output w
            | Constant1(w, _, _)
            | Constant(w, _)
            | Viewer w -> [ { LabName = compLbl; BitLimits = w - 1, 0 } ]
            | Demux2
            | Demux4
            | Demux8 -> [ { LabName = compLbl + "." + string outPortInt; BitLimits = 0, 0 } ]
            | NbitsXor w -> [ { LabName = compLbl; BitLimits = w - 1, 0 } ]
            | NbitsAdder w ->
                match outPortInt with
                | 0 -> [ { LabName = compLbl + ".Sum"; BitLimits = w - 1, 0 } ]
                | _ -> [ { LabName = compLbl + ".Cout"; BitLimits = w - 1, 0 } ]
            | DFF
            | DFFE -> [ { LabName = compLbl + ".Q"; BitLimits = 0, 0 } ]
            | Register w
            | RegisterE w -> [ { LabName = compLbl + ".Dout"; BitLimits = w - 1, 0 } ]
            | RAM1 mem
            | AsyncRAM1 mem
            | AsyncROM1 mem
            | ROM1 mem -> [ { LabName = compLbl + ".Dout"; BitLimits = mem.WordWidth - 1, 0 } ]
            | Custom c ->
            ...
            | SplitWire w ->
                ...
                let split { LabName = name; BitLimits = msb, lsb } st =
                    ...
                    |> function
                        | [], _ -> None
                        | lst, _ -> Some { LabName = name; BitLimits = List.max lst, List.min lst }
            ...
            | IOLabel ->
                ...
                let ioLblWidth =
                    FastRun.extractFastSimulationWidth fs (drivingComp.Id, []) (OutputPortNumber 0)
                match ioLblWidth with
                | None -> failwithf $"What? Can't find width for IOLabel {net[srcComp.Id].Label}$ "
                | Some width ->

                    [ { LabName = compLbl; BitLimits = width - 1, 0 } ]

            |> (fun composingLbls ->
                { OutputsAndIOLabels = netGroup2outputsAndIOLabels net netGrp
...
let netGroup2Label compIds (sd: SimulationData) netList (netGrp: NetGroup) =
    ...
    let tl =
        ...
        | lst when List.length lst > 0 ->
            let appendName st lblSeg =
                st
                + lblSeg.LabName
                + bitLimsString lblSeg.BitLimits
                + ", "
...
let makeLinePoints style (x1, y1) (x2, y2) =
    line (List.append style [ X1 x1; Y1 y1; X2 x2; Y2 y2 ]) []
...
let backgroundSvg (model: WaveSimModel) =
    ...
    |> Array.map (
        (fun x -> float x * model.SimParams.ClkSvgWidth)
        >> clkLine
    )
...
let private busLabelPositions (wSModel: WaveSimModel) (wave: Waveform) gaps =
    ...
    let nSpaces gap =
        (float gap.GapLen * clkWidth
         / (float maxBusValGap + 1.)
         + 2.)
    let busLabelXPosition g i =
        float g.GapStart
        + float i * float g.GapLen / float (nSpaces g)
    gaps
    |> Array.map (fun (gap) ->
...
let private busLabelRepeats wsMod (busLabelValAndPos: {| WaveValue: Sample; XPosArray: float[] |}) =
    let addLabel nLabels xInd =
        makeText (inWaveLabel nLabels xInd wsMod)
    match busLabelValAndPos.WaveValue with
    | Wire w when w.NBits > 1u ->
        Array.map
            (fun xInd ->
                addLabel 1 xInd (n2StringOfRadix true w.BitData w.NBits wsMod.WaveViewerRadix))
            busLabelValAndPos.XPosArray
    | _ -> [||]
...
let private makeSegment (clkW: float) (xInd: int) (data: Sample) (trans: int * int) =
    ...
    let makeSigLine = makeLinePoints [ Class "sigLineStyle"; Style [ Stroke("blue") ] ]
...
let waveSvg wsMod waveData =
    ...
    let padTrans t =
        ...
        | _ ->
            Array.pairwise t
            |> (fun pairs ->
                Array.concat [ [| 1, fst pairs[0] |]; pairs; [| snd (Array.last pairs), 1 |] ])
...
let addSVGToWaveSimModel wSModel =
    let waveData = getAllWaveSimDataBySample wSModel
    ...
    let lastRow =
        [| waveTableRow [ Class "fullHeight" ] (lwaveCell wSModel) (waveCellSvg wSModel true) bgSvg |]
    let firstRow =
        [| tr [ Class "rowHeight" ] [ td (waveCell wSModel) [ clkRulerSvg wSModel ] ] |]
...
let simulateAndMakeWaves (model: Model) (wsMod: WaveSimModel) (par: SimParamsT) : WaveSimModel =

    let newData =
        par.LastClkTime + 1u
        - uint (Array.length wsMod.SimDataCache)
    ...
    { wsMod with SimDataCache = newData; SimParams = par' }
...
let private isNLTrgtLstSelected
    (netList: NetList)
    ...
    List.exists
        ...
        comps
    || List.exists
        (fun (conn: Connection) -> isConnInNLTrgtLst (ConnectionId conn.Id) nlTrgtLst)
        conns

/// is the given waveform selected by the given selection
let private isNetGroupSelected
    (netList: NetList)
    ((comps, conns): CanvasState)
    (trgtLstGroup: NetGroup)
    =
...
let highlightConnectionsFromWaves (model: Model) (dispatch: Msg -> Unit) =
    ...
    | Some wSModel ->
        let waves =
            ...
            | WSEditorOpen
            | WSInitEditorOpen -> mapValues wSModel.AllWaves
            | WSViewerOpen ->
...
let fileMenuViewActions model dispatch =
    ...
    | _ -> highlightConnectionsFromWaves model dispatch
...
let cursorValueStrings (wSMod: WaveSimModel) =
    ...
    let makeCursVal sample =
        match sample with
        | Wire w when w.NBits > 1u ->
            [| n2StringOfRadix true w.BitData w.NBits paras.WaveViewerRadix |]
...
let isCursorVisible wSMod divWidth scrollPos =
    let cursLeftPos =
        cursorLeftPx wSMod.SimParams
        <| float wSMod.SimParams.CursorTime
    let cursMid =
        cursLeftPos
        + (wSMod.SimParams.ClkSvgWidth * 40.0 / 2.0)
    let leftScreenLim = scrollPos
    let rightScreenLim = leftScreenLim + divWidth
    cursLeftPos >= cursMid
    && cursMid <= rightScreenLim
...
let rec getSubSheets
    ...
    |> List.collect (fun sComp ->
        ...
        | Custom cusComp ->
            ...
            | None ->
                failwithf
                    "Custom compnent without simulation graph? InstancePath=%A, Instance=%A"
                    instances
                    label
            | Some sg ->
                let thisSheet =
                ...
                let otherSheets =
                    getSubSheets (sComp.Id :: sheets) (thisSheet.Instance :: instances) graph model
