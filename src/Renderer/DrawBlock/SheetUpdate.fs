...
let update (msg: Msg) (issieModel: ModelType.Model) : ModelType.Model * Cmd<ModelType.Msg> =
        ...
        let inputModel =
            ...
            |> (fun sMap ->
                ...
                ||> Map.fold (fun model sId sym ->
                    if
                        Map.containsKey sId model.BoundingBoxes
                        && sym.Pos = model.BoundingBoxes[sId].TopLeft
                    then
                        model
                    else
    ...
    | Wire(BusWireT.Symbol SymbolT.Msg.UpdateBoundingBoxes) ->
        ...
        model, Cmd.batch [ sheetCmd UpdateBoundingBoxes ]
    | Wire wMsg ->
        let wModel, (wCmd) = BusWireUpdate.update wMsg issieModel
        { model with Wire = wModel.Sheet.Wire }, wCmd
    | ToggleGrid -> { model with ShowGrid = not model.ShowGrid }, Cmd.none
    | KeyPress DEL ->
        let wiresConnectedToComponents =
        ...
        { model with
            ...
            RedoList = [] },
        Cmd.batch
            [ wireCmd (BusWireT.DeleteWires wireUnion) // Delete Wires before components so nothing bad happens
    ...
    | KeyPress CtrlV ->
        let newSymbolModel, pastedCompIds =
            SymbolUpdate.pasteSymbols model.Wire.Symbol model.Wire.Wires model.LastMousePos // Symbol has Copied Symbols stored
        let newBusWireModel, pastedConnIds =
            BusWireUpdate.pasteWires { model.Wire with Symbol = newSymbolModel } pastedCompIds

        { model with
            Wire = newBusWireModel
    ...
    | KeyPress CtrlZ ->
        ...
        | prevModel :: lst ->
            ...
            { prevModel with
                RedoList = appendRedoList model.RedoList model
                UndoList = lst
                CurrentKeyPresses = [] },
    ...
    | KeyPress CtrlA ->
        ...
        Cmd.batch
            [ symbolCmd (SymbolT.SelectSymbols symbols)
              wireCmd (BusWireT.SelectWires wires) ]

    | KeyPress CtrlW -> fitCircuitToScreenUpdate model
    ...
    | MouseMsg mMsg -> // Mouse Update Functions can be found above, update function got very messy otherwise
        let mouseAlreadyDown =
            match model.Action with
            | MovingPort _
            | ConnectingInput _
            | ConnectingOutput _ -> true
            | _ -> false
        match mMsg.Op with
    ...
    | UpdateScrollPos scrollPos ->
        ...
        let scrollDif =
            scrollPos
            - model.ScreenScrollPos * (1. / model.Zoom)
        ...
        recentProgrammaticScrollPos <-
            scrollPos
            :: List.truncate 4 recentProgrammaticScrollPos
    ...
    | AddNotConnected(ldcs, port, pos, rotation) ->
        let (newSymModel, ncID) =
            SymbolUpdate.addSymbol ldcs model.Wire.Symbol pos NotConnected ""
        ...
        let newWireModel, msgOpt =
            BusWireUpdate.newWire (InputPortId ncPortId) (OutputPortId port.Id) { model.Wire with Symbol = newSymModel }
        let wCmd =
            if msgOpt.IsSome then
                wireCmd (Option.get msgOpt)
            else
                Cmd.none
    ...
    | KeyPress ZoomIn ->
        ...
        { model with
            Zoom = min Constants.maxMagnification (model.Zoom * Constants.zoomIncrement) },
    ...
    | KeyPress ZoomOut ->
        ...
        { model with Zoom = newZoom }, sheetCmd (KeepZoomCentered oldScreenCentre)
    ...
    | ManualKeyDown key -> // Needed for e.g. Ctrl + C and Ctrl + V as they are not picked up by Electron
        let containsKey key =
            List.exists (fun (key', time) -> key' = key)
        let newPressedKeys =
            (key.ToUpper(), TimeHelpers.getTimeMs ())
            :: getActivePressedKeys model // Make it fully upper case to remove CAPS dependency
        //printfn $"Keys={newPressedKeys}, Key={key}"
        let newCmd =
            match
                containsKey "CONTROL" newPressedKeys
                || containsKey "META" newPressedKeys
            with
            | true ->
    ...
    | ManualKeyUp key ->
        ...
        let removeAllKeys key = List.filter (fun (k, t) -> k <> key)
        { model with
            CurrentKeyPresses = removeAllKeys (key.ToUpper()) model.CurrentKeyPresses },
        Cmd.none
    ...
    | CheckAutomaticScrolling ->
        ...
        let rightScreenEdge =
            leftScreenEdge + wholeApp.clientWidth
            - rightSelection.clientWidth
        ...
        let checkForAutomaticScrolling1D (edge: float) (mPos: float) (mMov: float) =
            ...
            if
                edgeDistance < scrollMargin && mMov >= -0.0000001 // just in case there are FP rounding errors
            then
                //printf "automaticScrolling adjustment..."
                scrollSpeed * (scrollMargin - edgeDistance)
                / scrollMargin // Speed should be faster the closer the mouse is to the screen edge
            else
                0.0
        canvas.scrollLeft <-
            canvas.scrollLeft
            - (checkForAutomaticScrolling1D leftScreenEdge mPosX -mMovX) // Check left-screen edge
        canvas.scrollLeft <-
            canvas.scrollLeft
            + (checkForAutomaticScrolling1D rightScreenEdge mPosX mMovX) // Check right-screen edge
        canvas.scrollTop <-
            canvas.scrollTop
            - (checkForAutomaticScrolling1D upperScreenEdge mPosY -mMovY) // Check upper-screen edge
        canvas.scrollTop <-
            canvas.scrollTop
            + (checkForAutomaticScrolling1D lowerScreenEdge mPosY mMovY) // Check lower-screen edge
        ...
        if xDiff <> 0.0 || yDiff <> 0.0 then // Did any automatic scrolling happen?
            let newMPos =
                { X = model.LastMousePos.X + xDiff / model.Zoom
                  Y = model.LastMousePos.Y + yDiff / model.Zoom }
            ...
            let outputModel, outputCmd =
                match model.Action with
                | DragAndDrop -> mMoveUpdate { model with AutomaticScrolling = true } { defaultMsg with Op = Move }
                | MovingSymbols
                | ConnectingInput _
                | ConnectingOutput _
                | Selecting -> mDragUpdate { model with AutomaticScrolling = true } { defaultMsg with Op = Drag }
                | _ -> { model with AutomaticScrolling = true }, Cmd.none
            let notAutomaticScrolling msg =
                match msg with
                | ModelType.Sheet CheckAutomaticScrolling -> false
                | _ -> true
            ...
            let filteredOutputCmd =
                Cmd.map
                    (fun msg ->
                        if notAutomaticScrolling msg then
                            msg
                        else
                            ModelType.Sheet DoNothing)
                    outputCmd
            // keep model ScrollPos uptodate with real scrolling position
            let outputModel =
                { outputModel with
                    ScreenScrollPos = { X = canvas.scrollLeft; Y = canvas.scrollTop } }

            outputModel, filteredOutputCmd
        else
            { model with AutomaticScrolling = false }, Cmd.none

    | Arrangement arrange -> arrangeSymbols arrange model

    | RotateLabels -> rotateSelectedLabelsClockwise model
    ...
    | Rotate rotation ->
        ...
        let rotmodel =
            { model with
                Wire =
                    { model.Wire with
                        Symbol = (RotateScale.rotateBlock model.SelectedComponents model.Wire.Symbol rotation) }
                TmpModel = Some model
                UndoList = appendUndoList model.UndoList model }

        let newModel =
            { rotmodel with BoundingBoxes = Symbol.getBoundingBoxes rotmodel.Wire.Symbol }
    ...
    | Flip orientation ->
        let flipmodel =
            { model with
                Wire =
                    { model.Wire with
                        Symbol = (RotateScale.flipBlock model.SelectedComponents model.Wire.Symbol orientation) }
                TmpModel = Some model
                UndoList = appendUndoList model.UndoList model }

        let newModel =
            { flipmodel with BoundingBoxes = Symbol.getBoundingBoxes flipmodel.Wire.Symbol }
    ...
    | SaveSymbols -> model, symbolCmd SymbolT.SaveSymbols
    ...
    | UpdateSelectedWires(connIds, on) ->
        ...
        let newWires =
            if on then
                ...
                List.filter (fun conId -> List.contains conId connIds |> not) oldWires
    ...
    | SetSpinner isOn ->
        if isOn then
            { model with CursorType = Spinner }, Cmd.none
        else
            { model with CursorType = Default }, Cmd.none
    ...
    | StartCompiling(path, name, profile) ->
        ...
        { model with
            ...
            DebugState =
                match profile with
                | Verilog.Debug -> Paused
                | Verilog.Release -> NotDebugging },
    ...
    | StartCompilationStage(stage, path, name, profile) ->
        ...
        printfn
            "do we have process? %A"
            (model.CompilationProcess
             |> Option.map (fun c -> c.pid))
        if not model.Compiling then
            ...
            let pcf, deviceType, devicePackage =
                ...
                | Some "IceStick", Verilog.Release -> $"{include_path}/icestick.pcf", "--hx1k", "tq144"

                | Some "IceStick", Verilog.Debug -> $"{include_path}/icestick_debug.pcf", "--hx1k", "tq144"

                | Some "IssieStick-v0.1", Verilog.Release -> $"{include_path}/issiestick-0.1.pcf", "--hx4k", "tq144"

                | Some "IssieStick-v0.1", Verilog.Debug -> $"{include_path}/issiestick-0.1_debug.pcf", "--hx4k", "tq144"

                | Some "IssieStick-v1.0", Verilog.Release -> $"{include_path}/issiestick-1.0.pcf", "--hx8k", "bg121"

                | Some "IssieStick-v1.0", Verilog.Debug -> $"{include_path}/issiestick-1.0_debug.pcf", "--hx8k", "bg121"
            ...
            let (prog, args) =
                ...
                | Synthesis ->
                    "yosys",
                    [ "-p"
                      $"read_verilog -I{include_path} {path}/{name}.v; synth_ice40 -flatten -json {path}/build/{name}.json" ] //"sh", ["-c"; "sleep 4 && echo 'finished synthesis'"]
                | PlaceAndRoute ->
                    "nextpnr-ice40",
                    [ "--package"
                      $"{devicePackage}"
                      ...
                      "--asc"
                      $"{path}/build/{name}.asc" ] //"sh", ["-c"; "sleep 5 && echo 'finisheded pnr'"]
                ...
                Async.StartImmediate(
                    async {
                        ...
                        try
                            ...
                            child.stderr.on ("data", (fun e -> printfn "Error: %s" e))
                            |> ignore
                            child.on (
                                "exit",
                                fun code ->
                            ...
                            if exit_code.Value = 0 then
                                ...
                                | Synthesis ->
                                    dispatchS
                                    <| StartCompilationStage(PlaceAndRoute, path, name, profile)
                                | PlaceAndRoute ->
                                    dispatchS
                                    <| StartCompilationStage(Generate, path, name, profile)
                                | Generate ->
                                    dispatchS
                                    <| StartCompilationStage(Upload, path, name, profile)
                    ...
                    try
                        ...
                        r.``then`` (fun v ->
                            ...
                            |> Array.iteri (fun i reading ->
                                ...
                                dispatch
                                <| ModelType.Sheet(
                                    OnDebugRead(hextoInt (reading[0].ToString() + reading[1].ToString()), i)
                                )))
                        |> ignore
    ...
    | DebugConnect ->
            ...
            Async.StartImmediate(
                async {
                    ...
                    try
                        ...
                        let c = simpleConnect ()
                        c.``then`` (fun v -> dispatch <| ModelType.Sheet(DebugRead viewers))
                        |> ignore
        ...
        { model with DebugIsConnected = true }, Cmd.ofSub (connectAndRead viewerNo)
    | DebugDisconnect ->
    ...
    | OnDebugRead(data, whichViewer) ->
        ...
        Cmd.none
    | DebugUpdateMapping mappings -> { model with DebugMappings = mappings }, Cmd.none
    | DebugContinue ->
    ...
    | SetDebugDevice device -> { model with DebugDevice = Some device }, Cmd.none

    | ToggleSnapToNet -> model, (wireCmd BusWireT.ToggleSnapToNet)
    ...
    | SetWireModel wModel -> { model with Wire = wModel }, Cmd.none
