...
module Constants =
    ...
    let valueOnWaveText =
        { DrawHelpers.defaultText with FontSize = fontSizeValueOnWave }
...
let topRowStyle =
    Style [ Height Constants.rowHeight; BorderBottom "2px solid rgb(219,219,219)" ]
...
let errorMessageStyle = Style [ Width "90%"; MarginLeft "5%"; MarginTop "15px" ]

/// Style of checkboxes
let checkboxStyle =
    Style [ Margin "0 5px 0 5px"; Cursor "pointer"; Float FloatOptions.Left ]

/// Props for Checkbox.Input
let checkboxInputProps: IHTMLProp list = [ Type "checkbox"; checkboxStyle ]

let boldFontStyle = Style [ FontWeight "bold"; FontSize "14px" ]

let normalFontStyle = Style [ FontWeight "normal"; FontSize "14px" ]

let noBorderStyle = Style [ BorderWidth 0 ]
...
let topHalfButtonProps color =
    [ Button.Color color; Button.Props [ topRowButtonStyle ] ]

let selectRamButtonProps = topHalfButtonProps IsInfo

/// Props for selectRamButton when no RAMs are selectable
let selectRamButtonPropsLight = selectRamButtonProps @ [ Button.IsLight ]
...
let selectWavesButtonPropsLight = selectWavesButtonProps @ [ Button.IsLight ]

let centerAlignStyle = Style [ TextAlign TextAlignOptions.Center; FontSize "15px" ]

/// Style for row in ramTable
let ramRowStyle =
    Style [ Height Constants.rowHeight; BorderBottom "1px solid rgb(219,219,219)" ]
...
let zoomOutSVG =
    svg
        [ ViewBox "0 0 192.904 192.904"; SVGAttr.Height "20px" ]
        [ path
              [ D
                    "M190.707,180.101l-47.079-47.077c11.702-14.072,18.752-32.142,18.752-51.831C162.381,36.423,125.959,0,81.191,0
          ...
          path
              [ D
                    "M118.035,73.689H44.346c-4.142,0-7.5,3.358-7.5,7.5c0,4.142,3.358,7.5,7.5,7.5h73.689c4.142,0,7.5-3.358,7.5-7.5
...
let zoomInSVG =
    svg
        ...
        [ path
          ...
          path
              [ D
                    "M118.035,73.689H88.69V44.345c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v29.345H44.346c-4.143,0-7.5,3.358-7.5,7.5
...
let valueOnWaveProps m i start width : IProp list =
    [ X(
          start * (singleWaveWidth m)
          + Constants.nonBinaryTransLen
          + float i * width
      )
...
let clkCycleInputProps: IHTMLProp list =
    [ Min 0; SpellCheck false; Step 1; clkCycleInputStyle ]
...
let clkCycleInnerStyle = Style(clkCycleBut @ [ BorderRadius 0 ])

/// Style for left-most clock cycle button
let clkCycleLeftStyle =
    Style(
        clkCycleBut
...
let nameRowLevelStyle isHovered =
    Style
        [ Height Constants.rowHeight
...
let nameRowLevelLeftProps (visibility: string) : IHTMLProp list =
    [ Style [ Position PositionOptions.Sticky; Left 0; Visibility visibility ] ]

let calcNamesColWidth (ws: WaveSimModel) : int =
    let cWidth =
        DrawHelpers.canvasWidthContext.font <-
            String.concat " " [ "10px"; Constants.columnFontFamily ] // e.g. "16px bold sans-serif";
        let getWidth (txt: string) =
            let sizeInPx = float ((Constants.columnFontSize.ToLower()).Replace("px", ""))
            sizeInPx
            * DrawHelpers.canvasWidthContext.measureText(txt).width
            / 10.0
        ws.SelectedWaves
        |> listCollectSomes (fun wi ->
...
let namesColumnStyle (ws: WaveSimModel) =
    Style(
        (waveSimColumn)
...
let valuesColumnStyle (colWidth: int) =
    Style(
        (waveSimColumn)
        @ [ MinWidth colWidth
            Float FloatOptions.Left
...
let waveformColumnStyle = Style [ GridColumnStart 2; Display DisplayOptions.Grid ]

/// Style for rows in waveforms column
let waveRowsStyle width =
    Style
        [ Height "100%"
...
let showWaveformsAndRamStyle =
    Style [ OverflowY OverflowOptions.Auto; Height "calc(100% - 250px)" ]

/// Style for waveforms only path of viewer
let showWaveformsStyle =
    Style
        [
...
let clkCycleSVGStyle =
    Style [ Display DisplayOptions.Block; BorderBottom Constants.borderProperties ]
...
let waveformColumnRowProps m : IProp list =
    [ SVGAttr.Height Constants.rowHeight
      ...
      ViewBox(
          ...
          + viewBoxWidth m
          + " "
          + string Constants.viewBoxHeight
      )
      PreserveAspectRatio "none" ]

/// Props for row of clock cycle numbers
let clkCycleNumberRowProps m : IProp list =
    waveformColumnRowProps m @ [ clkCycleSVGStyle ]

/// Style for each row in waveform column
let waveRowSVGStyle =
    Style [ Display DisplayOptions.Block; BorderBottom "1px solid rgb(219,219,219)" ]

/// Props for each row in waveform column
let waveRowProps m : IProp list =
    waveformColumnRowProps m @ [ waveRowSVGStyle ]

/// Style of line separating clock cycles
let clkLineStyle =
    Style [ Stroke "rgb(200,200,200)"; StrokeWidth Constants.clkLineWidth ]
    ...
    svg
        [ Style [ GridColumnStart 1; GridRowStart 1 ]
          ...
          ViewBox(
              ...
              + viewBoxWidth m
              + " "
              + string (Constants.viewBoxHeight * float (count + 1))
          )
          ...
          OnClick(fun ev ->
              ...
              let cycle =
                  (int <| (ev.clientX - bcr.left) / singleWaveWidth m)
                  + m.StartCycle
              dispatch
              <| UpdateWSModel(fun m -> { m with CurrClkCycle = cycle })) ]
        (List.append
            [ rect
...
let radixTabProps: IHTMLProp list =
    [ Style [ Width "35px"; Height Constants.rowHeight ] ]

/// Style for A HTML element in radixTabs
let radixTabAStyle = Style [ Padding "0 0 0 0"; Height Constants.rowHeight ]

/// Style for radixTabs
let radixTabsStyle =
    Style [ Height Constants.rowHeight; FontSize "80%"; Float FloatOptions.Right ]
      ...
      let clickHandler (e: Browser.Types.Event) =
          if isSummary then
              let show =
                  ...
                  | GroupItem(compGrp, subSheet) ->
                      Set.contains (compGrp, subSheet) ws.ShowGroupDetail
              dispatch
              <| UpdateWSModel(fun ws -> setSelectionOpen ws cBox (not show))
      let size, weight =
          match cBox with
...
let refreshSvg (color: string) (height: string) =
    svg
        [ ViewBox "0 0 512 512"; SVGAttr.Height height ]
        [ path
              [ D
                    "M496 48V192c0 17.69-14.31 32-32 32H320c-17.69 0-32-14.31-32-32s14.31-32
                ...
                Style [ Fill color; Stroke color; StrokeWidth "5px" ] ]
              [] ]

let emptyRefreshSVG = svg [ SVGAttr.Height "20"; SVGAttr.Width "20" ] []
...
let inline updateViewerWidthInWaveSim w (model: Model) =
    ...
    let otherDivWidths =
        ...
        + Constants.rightMargin
        + DiagramStyle.Constants.dividerBarWidth
    ...
    let viewerWidth =
        namesColWidth
        + Constants.valuesColWidth
        + int (singleCycleWidth * float wholeCycles)
        + otherDivWidths
...
let inline setViewerWidthInWaveSim w (model: Model) dispatch =
    dispatch
    <| UpdateModel(updateViewerWidthInWaveSim w)
