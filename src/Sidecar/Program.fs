...
let main _ =
    ...
    while true do
        ...
        // One connection at a time, served to completion here. Nothing needs a second: the
        // renderer asks everything of this simulator over its one socket, in operations it issues
        // one at a time. A second connection was accepted for a while, so that a BLOCKING read
        // could be made over plain HTTP - the only way to answer a question inside a render - and
        // it was removed with that: the answer arriving one render later costs nothing a user can
        // see, and it saved a second transport at both ends.
