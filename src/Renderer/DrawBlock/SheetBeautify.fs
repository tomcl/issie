...
let detectOverlaps (symbols: Symbol list) : (Symbol * Symbol) list =
    let boundingBoxes =
        symbols
        |> List.map (fun sym -> (sym, getSymBoundingBox sym))
    let overlaps =
        boundingBoxes
        |> List.collect (fun (sym1, box1) ->
            boundingBoxes
            |> List.filter (fun (sym2, box2) -> sym1 <> sym2 && overlap2DBox box1 box2)
...
let resolveOverlaps (overlaps: (Symbol * Symbol) list) : Symbol list -> Symbol list =
    let moveApart (sym1: Symbol, sym2: Symbol) : Symbol * Symbol =
        ...
        let moveDistanceX =
            if overlapX > 0.0 then
                overlapX + 1.0
            else
                0.0
        let moveDistanceY =
            if overlapY > 0.0 then
                overlapY + 1.0
            else
                0.0
    ...
    let updatedSymbols (symbols) =
        ...
        |> List.fold
            (fun acc (sym1, sym2) ->
                ...
                acc
                |> List.map (fun sym ->
                    if sym.Id = movedSym1.Id then
                        movedSym1
                    elif sym.Id = movedSym2.Id then
                        movedSym2
                    else
                        sym))
...
let alignSingleConnectedSyms (model: SheetT.Model) (syms) =
    ...
    let movedSyms =
        List.map (fun (s, t, w, b) -> moveSymbol (checkXCoor s t b |> delta w b) (if b then s else t)) syms
    let symbols' =
        (symbolMap, movedSyms)
    ...
    let nonOverlappingSymbols =
        resolveOverlaps overlaps (Map.toSeq symbols' |> Seq.map snd |> Seq.toList)

    let NewSymbolModel =
        Optic.set symbols_ (Map.ofSeq (List.zip (Map.keys symbols' |> Seq.toList) nonOverlappingSymbols)) model
    ...
    let movedWires =
        ...
        |> Map.values
        |> List.ofSeq
        |> List.map (fun w -> BusWireUpdateHelpers.autoroute NewSymbolModel.Wire w)
...
let sheetAlignScale (model: SheetT.Model) =
    ...
    printfn "Running SheetAlign %A"
    <| List.map (fun (s, t, w, b) -> (s.Component.Label, t.Component.Label, w.WId, b)) syms
