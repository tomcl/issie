...
module HLPTick3 =
    ...
    let middleOfSheet: XYPos = {X=maxSheetCoord/2.;Y=maxSheetCoord/2.}
...
//--------------------------------------------------------------------------------------------------//
    ...
    let randomPos: Gen<XYPos List> = 
        let x1Array = [-200..80..200] |> Array.ofList |> shuffleA
        let y1Array = [-200..80..200] |> Array.ofList |> shuffleA
        let x2Array = [-200..80..200] |> Array.ofList |> shuffleA
        let y2Array = [-200..80..200] |> Array.ofList |> shuffleA
        let x3Array = [-200..80..200] |> Array.ofList |> shuffleA
        let y3Array = [-200..80..200] |> Array.ofList |> shuffleA
        let xyPos1 = Array.map2 (fun x y -> {X=middleOfSheet.X + float x; Y=middleOfSheet.Y + float y}) x1Array y1Array
        let xyPos2 = Array.map2 (fun x y -> {X=middleOfSheet.X + float x; Y=middleOfSheet.Y + float y}) x2Array y2Array
        let xyPos3 = Array.map2 (fun x y -> {X=middleOfSheet.X + float x; Y=middleOfSheet.Y + float y}) x3Array y3Array
    ...
    let d2ComponentTypes: ComponentType list = 
        [Mux2; GateN(And,2); GateN(Or,2); GateN(Xor,2); GateN(Nand,2); GateN(Nor,2); GateN(Xnor,2); Mux2; Mux4; Mux2]
    ...
    let applyRandOperation (compType: ComponentType) (label: String) = 
    ...
    let makeRandomTestCircuit (posList:XYPos List) = 
        ...
        initSheetModel
            |> placeSymbol "C1" comp1 posList.[0]
            |> Result.bind (placeSymbol "C2" comp2 middleOfSheet)
            |> Result.bind (placeSymbol "C3" comp3 posList.[1])
            |> Result.bind (placeSymbol "C4" comp4 posList.[2])
            |> Result.bind (placeSymbol "S1" (Input1(1,None)) {X=middleOfSheet.X - 200.; Y=middleOfSheet.Y - 50.})
            |> Result.bind (placeSymbol "S2" (Input1(1,None)) {X=middleOfSheet.X - 200.; Y=middleOfSheet.Y - 100.})
            |> getOkOrFail
            |> applyRandOperation comp1 "C1"
            |> applyRandOperation comp2 "C2"
            |> applyRandOperation comp3 "C3"
            |> placeWire (portOf "C1" 0) (portOf "C2" (getRandPort comp2))
            |> Result.bind (placeWire (portOf "C2" 0) (portOf "C3" 0))
            |> Result.bind (placeWire (portOf "C3" 0) (portOf "C4" 0))
            |> Result.bind (placeWire (portOf "C4" 0) (portOf "C1" (getRandPort comp1)))
            |> Result.bind (placeWire (portOf "S1" 0) (portOf "C4" 1))
            |> Result.bind (placeWire (portOf "S2" 0) (portOf "C3" 1))
            |> getOkOrFail
    ...
    let makeTestCircuit1(andPos:XYPos) = 
        ...
        |> Result.bind (placeSymbol "2MUX2" Mux2 middleOfSheet)
        |> Result.bind (placeSymbol "3MUX2" Mux2 {X=middleOfSheet.X + 100.; Y=middleOfSheet.Y})
        |> Result.bind (placeSymbol "S1" (Input1(1,None)) {X=middleOfSheet.X - 100.; Y=middleOfSheet.Y})
        |> Result.bind (placeSymbol "S2" (Input1(1,None)) {X=middleOfSheet.X - 100.; Y=middleOfSheet.Y - 100.})
        |> getOkOrFail
        ...
        |> Result.bind (placeWire (portOf "2MUX2" 0) (portOf "3MUX2" 1))
        |> Result.bind (placeWire (portOf "S1" 0) (portOf "1MUX2" 0))
        |> Result.bind (placeWire (portOf "S2" 0) (portOf "2MUX2" 0))
        |> getOkOrFail
    ...
    let makeTest7Circuit (andPos:XYPos) =
        ...
        |> getOkOrFail
        |> SheetBeautifyHelpers.flipSymbol "G1" SymbolT.FlipType.FlipHorizontal
    ...
    let makeTestCircuit2 (andPos:XYPos) = 
        ...
        |> Result.bind (placeSymbol "G2" (GateN(And,2)) middleOfSheet)
        |> Result.bind (placeSymbol "G1" (GateN(And,2)) {X=middleOfSheet.X; Y=middleOfSheet.Y - 100.})
        |> Result.bind (placeSymbol "MUX1" Mux2 {X=middleOfSheet.X + 200.; Y=middleOfSheet.Y - 100.})
        |> Result.bind (placeSymbol "MUX2" Mux2 {X=middleOfSheet.X + 200.; Y=middleOfSheet.Y + 50.})
        |> Result.bind (placeSymbol "S1" (Input1(1,None)) {X=middleOfSheet.X - 100.; Y=middleOfSheet.Y - 150.})
        |> getOkOrFail
        ...
        |> Result.bind (placeWire (portOf "G2" 0) (portOf "MUX1" 0))
        |> Result.bind (placeWire (portOf "G2" 0) (portOf "MUX2" 1))
    ...
    let makeTestCircuit3 (andPos:XYPos) = 
        ...
        |> Result.bind (placeSymbol "S2" (Input1(1,None)) {X=middleOfSheet.X - 150.; Y=middleOfSheet.Y + 25.})
        |> Result.bind (placeSymbol "MUX1" Mux2 {X=middleOfSheet.X - 100.; Y=middleOfSheet.Y - 150.})
        |> Result.bind (placeSymbol "G1" (GateN(And,2)) {X=middleOfSheet.X + 100.; Y=middleOfSheet.Y - 100.})
        |> getOkOrFail
    ...
    module Tests =
        ...
        let test2 testNum firstSample dispatch =
            runTestOnSheets
                ...
                horizLinePositions
                makeTestCircuit2
                Asserts.indevTestWiresCrossing
                dispatch
            |> recordPositionInTest testNum dispatch
