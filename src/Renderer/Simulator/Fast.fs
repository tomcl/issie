        ...
        (fun fid fComp ->
            ...
            |> Array.iteri
                (fun iOut _ ->
                    ...
                    |> Array.iter
                        (fun (cid, opn, (InputPortNumber ipn)) ->
                            let linked =
                                Map.tryFind (cid, InputPortNumber ipn) linkCheck
                            ...
                            if isIOLabel fcDriven.FType then
                                let ioLabs =
                                    f.FIOLabels.[fcDriven.SimComponent.Label, ap]
...
let canBeReduced (step: int) (fc: FastComponent) =
    ...
    && not fc.Touched
    && not fc.Inactive
    && Array.forall (fun (arr: StepArray<FData>) -> arr.Step.Length > 0 && isValidData arr.Step.[0]) fc.InputLinks
...
let private orderCombinationalComponents (numSteps: int) (fs: FastSimulation) : FastSimulation =
    ...
    let orderedSet =
        ...
        |> Set
    /// this is the input set of all (fast0 components.
    ...
    let badComps =
        ...
        |> List.filter
            (function
            | { Inactive = inactive } -> not inactive)
...
let private stepSimulation (fs: FastSimulation) =
    Array.iter
        ...
        (Array.concat [ fs.FConstantComps
                        fs.FGlobalInputComps
                        fs.FClockedComps
                        fs.FOrderedComps ])
...
let runFastSimulation (numberOfSteps: int) (fs: FastSimulation) : Unit =
    if numberOfSteps > fs.MaxStepNum then
        let newMaxNum =
            numberOfSteps
            + max 50 (int (float numberOfSteps * 1.5))
...
let compareFastWithGraph (sd: SimulationData) =
    ...
    assertThat
        ...
        (sprintf
            "Can't compare FastSim because it is on step %d when simulationdata is step %d"
            fs.ClockTick
            sd.ClockTickNumber)
    ...
    |> List.collect
        (fun ((cid, ap), fc) ->
            if isIOLabel fc.FType then
                ...
                |> Map.toList
                |> List.collect
                    (fun ((InputPortNumber n), wd) ->
                        let wd' =
                            fc.InputLinks.[n].Step.[sd.ClockTickNumber]

                        if wd <> wd' then
                            let msg =
                                sprintf "Data Error: %25s:In%d fast=<%A> old=<%A>" fc.FullName n wd' wd
