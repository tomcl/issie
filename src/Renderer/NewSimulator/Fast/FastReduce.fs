...
let inline extractBitFData (fd_: FData) (busWidth: int) : uint32 =
    ...
    | Data fd ->
        ...
        | BigWord _ ->
            failwithf
                $"Can't extract %d{busWidth} bit from BigWord data {fd.Dat} of width {fd.Width}"
...
let inline extractBit (fd: FastData) (busWidth: int) : uint32 =
    ...
    | Word n -> n
    | BigWord _ ->
        failwithf $"Can't extract %d{busWidth} bit from BigWord data {fd.Dat} of width {fd.Width}"
...
let fastReduce
    ...
    let inline getLastCycleOut n =
        let fd =
            ...
            | None, _ ->
                failwithf
                    "Can't reduce %A (%A) because outputwidth is not known"
                    comp.FullName
                    comp.FType
            | Some w, 0 ->
                if w < 33 then
    ...
    | NbitsAdderNoCout numberOfBits ->
        ...
        | Data cin, Data A, Data B ->
            let sum, cout =
                ...
                | a, b ->
                    failwithf
                        $"Inconsistent inputs to NBitsAdder {comp.FullName} A={a},{A}; B={b},{B}"
    ...
    | NbitsAnd numberOfBits ->
        ...
        | Data A, Data B ->
            let outDat =
                ...
                | a, b ->
                    failwithf
                        $"Inconsistent inputs to NBitsAnd {comp.FullName} A={a},{A}; B={b},{B}"
    ...
    | SplitWire topWireWidth ->
        ...
        <| sprintf
            "SplitWire received too few bits: expected at least %d but got %d"
            (topWireWidth + 1)
            fd.Width
#endif
        match fd with
    ...
    | RegisterE width ->
        ...
        | Data bits, Data enable ->
            ...
            <| sprintf
                "RegisterE received data with wrong width: expected %d but got %A"
                width
                bits.Width
#endif
    ...
    | Counter width ->
        ...
        | Data bits, Data load, Data enable ->
            ...
            assertThat (bits.Width = width)
            <| sprintf
                "Counter received data with wrong width: expected %d but got %A"
                width
                bits.Width
#endif
            if
    ...
    | AsyncROM1 mem -> // Asynchronous ROM.
        ...
        <| sprintf
            "ROM received address with wrong width: expected %d but got %A"
            mem.AddressWidth
            fd
#endif
        match fd with
    ...
    | RAM1 memory ->
        ...
        | Data address, Data dataIn ->
            ...
            <| sprintf
                "RAM received address with wrong width: expected %d but got %A"
                mem.AddressWidth
                address
#endif
#if ASSERTS
            ...
            <| sprintf
                "RAM received data-in with wrong width: expected %d but got %A"
                mem.WordWidth
                dataIn
#endif
...
let fastReduceFastData
    ...
    | Mux8 ->
        let fd0, fd1, fd2, fd3, fd4, fd5, fd6, fd7, bitSelect =
