...
let inCon2str con =
    match con with
    | Equality e -> sprintf "%s = 0x%X" (e.IO.getLabel) e.Value
    | Inequality i -> sprintf "0x%X \u2264 %s \u2264 0x%X" i.LowerBound (i.IO.getLabel) i.UpperBound
...
let makeElementLine (elsLeftAlign: ReactElement list) (elsRightAlign: ReactElement list) =
    ...
    Level.level [] [ Level.left [] itemListLeft; Level.right [] itemListRight ]
...
let viewNumericalConstraints cons dispatch =
    ...
    let equEls =
        cons.Equalities
        |> List.map (fun con -> con |> Equality |> makeConTag)
    let inequEls =
        cons.Inequalities
        |> List.map (fun con -> con |> Inequality |> makeConTag)
...
let constraintsOverlap (con1: Constraint) (con2: Constraint) =
    let equAndIneqOverlap (equ: EqualityConstraint) (ineq: InequalityConstraint) =
        equ.IO = ineq.IO
        && equ.Value >= ineq.LowerBound
        && equ.Value <= ineq.UpperBound

    let checkTwoIneq (in1: InequalityConstraint) (in2: InequalityConstraint) =
        in1.IO = in2.IO
        && ((in1.LowerBound >= in2.LowerBound
             && in1.LowerBound <= in2.UpperBound)
            || (in1.UpperBound >= in2.LowerBound
                && in1.UpperBound <= in2.UpperBound))
    ...
    | Inequality c1, Inequality c2 -> (checkTwoIneq c1 c2) || (checkTwoIneq c2 c1)

/// Check if a newly added numerical constraint is valid and consistent with current constraints
let validateNumericalConstraint
    (con: Constraint)
            ...
            ||> List.fold (fun state c ->
                ...
                | Ok eqc ->
                    if constraintsOverlap con (Inequality c) then
                        ...
                        sprintf
                            "This constraint overlaps with another constraint: %s.
                        Please change your new constraint or delete the old one."
                            constr
                        |> Error
                    else
                        Ok eqc)
        ...
        *)
        ...
        let constraintEditor =
            ...
            | Some Equ, Some io ->
                ...
                makeElementLine
                    [ str
                      <| sprintf "%s = " (SimulationView.makeIOLabel (string label) width)
                      numField1 width ]
                    []
            | Some Ineq, Some io ->
                let label, width = io.getLabel, io.getWidth
        ...
        match
            dialogData.Int,
            dialogData.Int2,
            dialogData.ConstraintErrorMsg,
            dialogData.ConstraintIOSel,
            dialogData.ConstraintTypeSel
        with
        ...
        | Some lower, Some upper, None, Some io, Some Ineq ->
            let tentative =
                Inequality
                <| makeInequalityConstraint lower io (int upper)
            match validateNumericalConstraint tentative existingCons with
            | Error err ->
                err
                |> Some
            ...
            | Ok c ->
                None |> SetPopupConstraintErrorMsg |> dispatch
                c |> Some |> SetPopupNewConstraint |> dispatch
        | _, _, _, _, _ -> None |> SetPopupNewConstraint |> dispatch
...
let createInputConstraintPopup (model: Model) (dispatch: Msg -> Unit) =
    ...
    let body =
        dialogPopupNumericalConBody inputs model.TTInputConstraints infoMsg dispatch
    let buttonText = "Add"
    let buttonAction =
        fun (dialogData: PopupDialogData) ->
...
let createOutputConstraintPopup (model: Model) (dispatch: Msg -> Unit) =
    ...
    let infoMsg =
        "If the truth table is truncated, results from applying Output Constraints may not be complete."
    let outputs =
        match model.CurrentTruthTable with
        | None -> failwithf "what? No current Truth Table when adding Constraints"
        | Some(Error _) ->
        ...
        | Some(Ok tt) ->
            ...
            |> List.map (fun cell -> cell.IO)
    let body =
        dialogPopupNumericalConBody outputs model.TTOutputConstraints infoMsg dispatch
    let buttonText = "Add"
    let buttonAction =
        fun (dialogData: PopupDialogData) ->
            match dialogData.NewConstraint with
...
let algebraKey =
    div
        [ Style [ OverflowY OverflowOptions.Scroll ] ]
        [ str
              "Algebraic expressions in Issie consist of input variables, numeric values,
          ...
          str
              "Variables in algebraic expressions correspond the input components in your
          ...
          br []
          str
              "Represents the addition of two expressions, just like it would in maths.
          ...
          br []
          str
              "Represents the subtraction of two expressions, just like it would in maths.
                The width of the result is equal to the width of the operands. If the result
          ...
          br []
          str
              "Represents an AND operation between two expressions. This is a bitwise operation,
          ...
          br []
          str
              "Represents an OR operation between two expressions. This is a bitwise operation,
                meaning that the operator is applied to each bit one-by-one. For example, for inputs
          ...
          br []
          str
              "Represents an XOR operation between two expressions. This is a bitwise operation,
          ...
          br []
          str
              "Represents the selection of a specific range of bits from an expression. The upper
          ...
          br []
          str
              "Represents the joining of two expressions together, with the first expression making
          ...
          br []
          str
              "Compares the value of an algebraic expression to a number. If the equality holds (true)
...
let validateAlgebraInput (io: SimulationIO) (fsi: FSInterface) (tableSD: SimulationData) =
    ...
    with AlgebraNotImplemented err ->
        Error err
...
let dialogPopupReductionBody inputs tableSD (dispatch: Msg -> unit) =
    fun (dialogData: PopupDialogData) ->
        ...
        let error =
            ...
            | Some { Msg = m
                     InDependency = (Some d)
                     ComponentsAffected = _
                     ConnectionsAffected = _ } ->
                div [] [ str m; br []; str $"Component in question: {d}" ]
            | _ ->
                failwithf
                    "what? SimError for AlgebraNotImplemented should always have an
                    InDependency field. Error: %A"
                    dialogData.AlgebraError

        div [] [ explanation; hr []; toggles; hr []; error ]
...
let createAlgReductionPopup model dispatch =
    ...
    let inputs, tableSD =
        ...
        | Some(Error _) ->
            failwithf "what? Reduction option should not exist when there is TT error"
        | Some(Ok tt) ->
            tt.TableMap
            |> Map.toList
            |> List.map fst
...
let viewReductions (table: TruthTable) (model: Model) dispatch =
    let goBackButton =
        ...
        | Some _, [] ->
            (Button.button
                [ Button.Color IsInfo; Button.OnClick(fun _ -> dispatch ClearDCMap) ]
    ...
    let reduceButton =
        if table.IsTruncated then
            ...
            (Button.button [ Button.Disabled true; Button.OnClick(fun _ -> ()) ] [ textEl ])
            ...
            (Button.button
                [ Button.Color IsSuccess; Button.OnClick(fun _ -> dispatch DCReduceTruthTable) ]
                [ str "Remove Redundancies" ])
    let algebraButton =
        Button.button
            [ Button.Color IsSuccess
              Button.OnClick(fun _ -> createAlgReductionPopup model dispatch) ]
            [ str "Algebra" ]
    let keyButton =
        Button.button
            [ Button.Color IsInfo; Button.OnClick(fun _ -> createAlgKeyPopup dispatch) ]
            [ str "Key" ]
    let algebraTags =
    ...
    | None, [] -> // Table is neither DC Reduces or Algebraic
        div [] [ makeElementLine [ reduceButton; str "   "; algebraButton ] [ maybeBaseSelector ] ]
    | Some dc, [] -> // Table is DC Reduced
        div [] [ makeElementLine [ goBackButton ] [ maybeBaseSelector ] ]
    | None, _ :: _ -> // Table is Algebraic
        div
            []
            [ makeElementLine
                  [ goBackButton; str "   "; algebraButton; str "   "; keyButton ]
                  [ maybeBaseSelector ]
