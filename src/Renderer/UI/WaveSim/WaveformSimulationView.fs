...
let maxUsedViewerWidth (wSMod: WaveSimModel) =
    ...
    let namesColWidth =
        ...
        | [||] -> 0.0
        | wN -> Array.map strWidth wN |> Array.max |> max 100.0
    let svgWaveColWidth =
...
let private changeWaveConnsSelect
    (selFun: WaveformSpec -> bool)
    (model: Model)
    ...
    | ViewerWaveform _ ->
        let wSModel' =
            { wSModel with
                AllWaves = Map.add wave.WId { wave with WType = ViewerWaveform on } wSModel.AllWaves }
        dispatch <| SetWSModel wSModel'
    | NormalWaveform -> selectWaveConns model (selFun wave) wave dispatch

/// toggle selection of a waveform's connections
let private toggleWaveConnsSelect
    (model: Model)
    (wSMod: WaveSimModel)
    (name: string)
    (dispatch: Msg -> unit)
...
let private changeViewerZoom compIds plus (m: Model) (wsModel: WaveSimModel) dispatch =
    let rec adjustLastClk viewW wsModel =
        ...
        if viewW * 1.4 < float (maxUsedViewerWidth wsModel) then
            adjustLastClk
                viewW
                (setSimParams (fun sp -> { sp with LastClkTime = pars.LastClkTime + 2u }) wsModel)
        else
    ...
    let newPars =
        ...
        | true ->
            { pars with
                ...
                ClkSvgWidth = newClkW }
        | false -> { pars with ClkSvgWidth = newClkW }
    dispatch
    <| InitiateWaveSimulation(WSViewerOpen, newPars)
    ...
    | true, false ->
        let pars' =
            { pars with
                ...
                LastClkTime = pars.LastClkTime }
        dispatch
        <| InitiateWaveSimulation(WSViewerOpen, pars')
        UpdateScrollPos true |> dispatch
    | false, _ -> ()
...
let private moveWave (model: Model) (wSMod: WaveSimModel) up =
    ...
    let movedNames =
        ...
        |> Array.fold
            (fun (arr, prevSel) (sel, p) ->
                match sel, prevSel with
                | true, true -> addLastPort arr p, sel
                | s, _ -> Array.append arr [| s, [| p |] |], s)
            ([||], false)
        |> fst
        |> Array.mapi (fun i (sel, ports) ->
            if sel then
                float i + moveBy, ports
            else
                float i, ports)
        |> Array.sortBy fst
        |> Array.collect snd
    setDispNames movedNames wSMod |> SetWSModel

let private viewerWaveSet
    (wavesToSet: WaveformSpec array)
    (on: bool)
    (wSMod: WaveSimModel)
    dispatch
    =
    let waves =
        ...
        |> Map.map (fun wid wave ->
            ...
            | _ -> wave)
    dispatch
    <| SetWSModel { wSMod with AllWaves = waves }
...
let private waveAdderSelectAll model (wSMod: WaveSimModel) (on: bool) dispatch =
    ...
    let conns =
        match on with
        | true -> waves |> Array.collect (fun wave -> wave.Conns)
        | false -> [||]
...
let showSimulationLoading (wsModel: WaveSimModel) (dispatch: Msg -> Unit) =
    ...
    let resetV waves =
        ...
        |> Map.map (fun wid wave ->
            match wave.WType with
            | ViewerWaveform _ -> { wave with WType = ViewerWaveform false }
            | _ -> wave)
    match nv, v with
    | None, _ -> false
    | Some _, _ ->
        ...
        dispatch
        <| UpdateWSModel(fun wsm -> { wsm with AllWaves = resetV wsm.AllWaves })
        true
...
let private makeRamReactCol (wsModel: WaveSimModel) ramPath =
    ...
    [| label
           [ Class "ramVals" ]
           [ p [ Style [ FontWeight "Bold"; TextAlign TextAlignOptions.Center ] ] [ str ramName ] ] |]
    :: List.map makeReactEl ramLocs
    |> List.toArray
        ...
        |> Array.map (fun (name, lab) ->
            ...
            tr
                ...
                [ td
                  ...
                  td [ Class "waveNamesCol"; Style [ TextAlign TextAlignOptions.Right ] ] [ lab ] ])
...
let private radixTabs (wsModel: WaveSimModel) dispatch =
    let radixString =
        [ Dec, "uDec"; Bin, "Bin"; Hex, "Hex"; SDec, "sDec" ]
        |> Map.ofList

    let radTab rad =
        Tabs.tab
            [ Tabs.Tab.IsActive(wsModel.SimParams.WaveViewerRadix = rad)
              Tabs.Tab.Props [ Style [ Width "35px"; Height "30px" ] ] ]
            [ a
                  [ Style [ CSSProp.Padding "0 0 0 0"; Height "30px" ]
                    OnClick(fun _ ->
                        InitiateWaveSimulation(
                            WSViewerOpen,
                            { wsModel.SimParams with WaveViewerRadix = rad }
                        )
                        |> dispatch) ]
                  [ str (radixString[rad]) ] ]
    Tabs.tabs
    ...
    div
        ...
        [ Button.button
              [ Button.CustomClass "cursLeft"
                Button.OnClick(fun _ -> cursorMove false wSMod dispatch) ]
              [ str "◀" ]
          Input.number
...
let private cursorValuesCol rows =
    let rightCol =
        Array.append [| tr [ Class "rowHeight" ] [ td [ Class "rowHeight" ] [] ] |] rows
    div
        [ Style
              [ Float FloatOptions.Right
                Height "100%"
        ...
        th
            ...
            [ Button.button
                  [ Button.CustomClass "newWaveButton"
                    ...
                    Button.OnClick(fun _ -> openEditorFromViewer model dispatch) ]
                  [ str "Edit list..." ] ]
                     ...
                     [ div
                           ...
                           [ Button.button
                                 [ Button.CustomClass "updownBut"
                                   Button.OnClick(fun _ -> moveWave model wsMod true |> dispatch) ]
                                 [ str "▲" ]
                             Button.button
                                 [ Button.CustomClass "updownBut"
                                   Button.OnClick(fun _ -> moveWave model wsMod false |> dispatch) ]
                                 [ str "▼" ] ] ]
                 waveAddDelBut ] |]

    let bot =
        [| tr [ Class "fullHeight" ] [ td [ Class "checkboxCol" ] []; td [] [] ] |]

    let leftCol = Array.concat [| top; labelRows; bot |]

    div
        [ Style [ Float FloatOptions.Left; Height "100%" ] ]
...
let private allWaveformsTableElement model (wSModel: WaveSimModel) waveformSvgRows dispatch =
    ...
    let allWaveformsHtmlRef (el: Browser.Types.Element) =
        ...
        | Some e ->
            ...
            | _ -> ()
        | None -> UpdateScrollPos false |> dispatch
    ...
    let scrollFun (ev: Browser.Types.UIEvent) = // function called whenever scroll position is changed
        ...
        | None -> () // do nothing
        | Some e ->
            if e.scrollWidth - e.clientWidth - e.scrollLeft < 10.0 then
                let pars' =
                    { pars with
                        ClkSvgWidth = pars.ClkSvgWidth
    ...
    div
        [ Ref allWaveformsHtmlRef
          OnScroll scrollFun
          Style [ MaxWidth(maxWavesColWidth wSModel); MinHeight "100%" ]
          Class "wavesTable" ]
        [ div [ Class "cursorRectStyle"; cursRectStyle wSModel.SimParams ] [ str " " ]
          table
              ...
              [ tbody
                    [ Style [ Height "100%" ] ]
                    (Array.concat [| waveformSvgRows.Top; waves; waveformSvgRows.Bottom |]) ] ]

let waveSimViewerRamDisplay wSMod ramPath =
    makeRamReactCol wSMod ramPath
    |> Array.map (fun c -> tr [] [ td [ Class "cursValsCol" ] c ])
    |> (fun r ->
        [| div [ Style [ Height "calc(100vh - 150px)"; OverflowY OverflowOptions.Auto ] ] r |])
...
let private viewWaveformViewer compIds model wSMod (dispatch: Msg -> unit) =
    let tableWaves, nameColMiddle, cursValsRows =
        waveSimViewerRows compIds model wSMod dispatch
    div
        [ Style [ Height "calc(100% - 45px)"; Width "100%"; OverflowY OverflowOptions.Auto ] ]
        ((List.map
            (fun ramPath -> cursorValuesCol (waveSimViewerRamDisplay wSMod ramPath))
...
let private viewZoomDiv compIds model wSMod dispatch =
    div
        [ Class "zoomDiv" ]
        [ button
              [ Button.Option.IsOutlined; Button.CustomClass "zoomButLeft" ]
              (fun _ -> changeViewerZoom compIds false model wSMod dispatch)
              "-"
          button
              [ Button.CustomClass "zoomButRight" ]
              (fun _ -> changeViewerZoom compIds true model wSMod dispatch)
              "+" ]
...
let sortEditorNameOrder wsModel =
    let otherNames =
        wsModel.AllWaves
        |> Map.filter (fun wId wSpec ->
            not
            <| Array.contains wId wsModel.SimParams.DispNames)
...
let private waveEditorTickBoxesAndNames (model: Model) wSModel (dispatch: Msg -> unit) =
    div
        [ Style [ Position PositionOptions.Relative; CSSProp.Top "20px" ] ]
        [ table
              []
              [ tbody
                    []
                    (Array.append
                        [| waveEditorSelectAllRow model wSModel dispatch |]
...
let private waveEditorButtons (model: Model) (wSModel: WaveSimModel) dispatch =
    ...
    let moreWaveEditorButtonAction _ =
        dispatch
        <| SetPopupWaveSetup(getWaveSetup wSModel model)
        PopupView.showWaveSetupPopup
            (Some "RAM Viewer Setup: tick to view RAM or ROM contents")
            (getWavePopup dispatch)
            None
            []
            dispatch
    ...
    let waveEditorViewSimButtonAction =
        ...
        let viewableWaves =
            ...
            |> standardOrderWaves wSModel.SimParams.DispNames (fun s ->
                Map.containsKey s allWaves
                && isWaveSelected model allWaves[s])
    ...
    let cancelButton =
        Button.button
            [ Button.Color IsSuccess; Button.OnClick(closeWaveSimButtonAction) ]
            [ str "Close" ]
    ...
    let moreButton =
        Button.button
            [ Button.Color IsSuccess
              ...
              Button.OnClick(moreWaveEditorButtonAction) ]
            [ str "RAM" ]
...
let private waveEditorView model wSMod (dispatch: Msg -> unit) =
    [ div
          ...
          [ Heading.h4 [] [ str "Waveform Simulation" ]
            str
                "Ctrl-click on diagram connections or use tick boxes below to add or remove waveforms."
...
let private openEditorFromViewer model (dispatch: Msg -> Unit) : Unit =
    ...
    dispatch
    <| UpdateWSModel(setEditorView WSEditorOpen)
...
let startWaveSim
    ...
    let compIds =
        comps
        |> List.map (fun c -> ComponentId c.Id)
        |> Set
    ...
    let inputWarningPopup (simData: SimulatorTypes.SimulationData) dispatch =
        if simData.Inputs <> [] then
            let inputs =
                ...
                |> String.concat ","
            let popup =
                Notifications.warningPropsNotification (
                    sprintf "Inputs (%s) will be set to 0." inputs
                )
            dispatch <| SetPropertiesNotification popup

    let startingWsModel =
        let modelWithWaveSimSheet =
            { model with WaveSimSheet = Option.get (getCurrFile model) }
        let wsModel =
            getWSModelOrFail
                modelWithWaveSimSheet
                "What? Can't get wsModel at start of new simulation"
        ...
        let waves =
            allowedNames
            |> Array.map (fun name -> wSpecs[name])
        Array.iter (fun wave -> selectWaveConns model true wave dispatch) waves
        let wSpecs =
            wSpecs
    ...
    dispatch
    <| SetWSModAndSheet(startingWsModel, Option.get (getCurrFile model))
    dispatch <| SetViewerWidth minViewerWidth
    dispatch
    <| SetLastSimulatedCanvasState(Some rState)
...
let WaveformButtonFunc compIds model dispatch =
    ...
    let simulationButton =
        ...
        | Some wSModel ->
            match
                wSModel.WSViewState,
                model.WaveSimulationIsOutOfDate,
                          ...
                          Button.OnClick(fun _ ->
                              let popup =
                                  Notifications.errorPropsNotification
                                      "Combinational logic does not have waveforms"
                              dispatch <| SetPropertiesNotification popup) ]
            ...
            | x, y, z ->
                ...
                Button.button
                    [ Button.Color IsSuccess; Button.IsLight; Button.OnClick(fun _ -> ()) ]
        ...
        | _, prog ->
            ...
            [ div
                  ...
                  [ Heading.h4 [] [ str "Waveform Simulation" ]
                    ...
                    str
                        "Test combinational logic by closing this simulator and using Step Simulator tab."
                    hr []
                    br []
                    button ] ]
              ...
              [ SimulationView.viewSimulationError simError
                button
                    ...
                    (fun _ ->
                        ...
                        | Some ws ->
                            dispatch
                            <| SetWSModel { ws with InitWaveSimGraph = None }
                        | _ -> ()
