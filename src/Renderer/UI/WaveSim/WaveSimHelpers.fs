...
let sheetOfWave (fs: FastSimulation) (wi: WaveIndexT) : string option =
    match snd wi.Id with
    | [] ->
...
let waveToSheetPort fs (wi: WaveIndexT) =
    sheetOfWave fs wi
...
let connsOfWave (fs:FastSimulation) (wi: WaveIndexT) =
    wi
    |> waveToSheetPort fs
...
///
/// Nothing here clears the schematic. It used to reset the sheet's selection, because that is
/// where the hover highlight had put itself; the highlight is now derived from the viewer's own
/// state and goes with it, and the user's selection is the user's.
