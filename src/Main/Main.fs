...
let startRenderer (doAfterReady: BrowserWindow -> Unit) =
    mainProcess.app.on_ready(fun _ _ ->
        ...
        // The dotnet sidecar starts with the app; its port is answered by issie:sidecarPort once
        // the process has reported in.
        //
        // In front of the splash deliberately, and it was not always affordable there: while the
        // sidecar was published as a single file it had to unpack itself inside CreateProcess, and
        // spawning it blocked here for 265ms - a quarter of a second in front of the first thing
        // the user sees. Published as a directory it is 5ms (see scripts/publish-sidecar.js), and
        // 5ms buys the simulator being ready at ~0.3s rather than ~2.6s, which is what someone who
        // presses Simulate as soon as the window appears is waiting for.
        Bridge.startSidecar ()
