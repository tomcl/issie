...
let matchMouseMsg (msgSelect: DrawHelpers.MouseT -> bool) (msg: Msg) : bool =
    ...
    | Sheet sMsg ->
        match sMsg with
        | SheetT.MouseMsg mMsg -> msgSelect mMsg
        | _ -> false
    | _ -> false
...
let shortDWSM (ws: WaveSimModel) =
    Some
    <| sprintf
        $"WS<{ws.FastSim.SimulatedTopSheet}->{ws.StartCycle}-{ws.CurrClkCycle}-\
...
let traceMessage startOfUpdateTime (msg: Msg) ((model, cmdL): Model * Cmd<Msg>) =
    if JSHelpers.debugLevel > 0 then
        ...
        let rootOfMsg =
            ...
            | _ -> ""
        TimeHelpers.instrumentInterval rootOfMsg startOfUpdateTime
        |> ignore
        let updateTime = TimeHelpers.getTimeMs () - startOfUpdateTime
        updateTimeTotal <-
            match updateTimeTotal > 1000. with
            | true -> 0.
            | false -> updateTimeTotal + updateTime
        ...
        if Set.contains "update" JSHelpers.debugTraceUI then
            ...
            TimeHelpers.instrumentInterval logMsg (startOfUpdateTime) msg
            |> ignore
...
let updateAllMemoryCompsIfNeeded (model: Model) =
    ...
    if
        time - lastMemoryUpdateCheck > Constants.memoryUpdateCheckTime
        && (getWSModel model).State = Success
    then
...
let getContextMenu (e: Browser.Types.MouseEvent) (model: Model) : string =
    ...
    let sheetXYPos =
        SheetDisplay.getDrawBlockPos e DiagramStyle.getHeaderHeight model.Sheet
    ...
    let mouseInScalingBox =
        let insideBox (pos: XYPos) boundingBox =
            let { BoundingBox.TopLeft = { X = xBox; Y = yBox }; H = hBox; W = wBox } =
                boundingBox
            pos.X >= xBox - 50.0
            && pos.X <= xBox + wBox + 50.0
            && pos.Y >= yBox - 50.0
            && pos.Y <= yBox + hBox + 50.0
    ...
    rightClickElement <-  // mutable so that we have this info also in the callback from main
        ...
        | _, "selectButton", _
        | _, "selectRamButton", _
        | _, "startEndButton", _ -> WaveSimHelp
        | _, elId, _ when String.startsWith "SheetMenuBreadcrumb:" elId ->
            ...
            |> Option.map (fun p ->
                ...
                |> Option.map (fun sheet -> SheetMenuBreadcrumb(sheet, nameParts.Length > 2)))
            |> Option.flatten
            |> Option.defaultValue NoMenu

        | SheetT.MouseOn.Canvas, _, "path"
        | _, "WaveSimHelp", _ -> WaveSimHelp
        ...
        | SheetT.MouseOn.Component compId, _, _ ->
            if mouseInScalingBox then
                ...
                | Some { Component = { Type = Custom ct } } -> DBCustomComp(symbols[compId], ct)
                | Some sym when sym.Annotation = None -> DBComp sym
                | _ -> NoMenu
    ...
    | SheetMenuBreadcrumb _ ->
        if JSHelpers.debugLevel > 0 then
            "SheetMenuBreadcrumbDev"
        else
            "SheetMenuBreadcrumbDev"
    | DBScalingBox _ -> "ScalingBox"
    | DBCustomComp _ -> "CustomComponent"
    | DBComp _ -> "Component"
    | DBCanvas _ -> "Canvas"
    | DBWire _ -> "Wire"
    | WaveSimHelp -> "WaveSimHelp"
    | _ ->
...
let processContextMenuClick
    ...
    (model: Model)
    : Model * Cmd<Msg>
    = // can change state directly (Model) or via a message wrapped in Cmd.ofMsg.
    ...
    let withWireMsg msg =
        withMsg (Msg.Sheet(SheetT.Msg.Wire msg))
    ...
    | DBComp sym, "Properties"
    | DBCustomComp(sym, _), "Properties" ->
        model
    ...
    | DBComp sym, "Copy (Ctrl+C)" ->
        let model =
            if model.Sheet.SelectedComponents = [] then // make sure at least one symbol is selected for copy
                model
                |> map (sheet_ >-> SheetT.wire_ >-> BusWireT.symbol_) (fun model ->
                    SymbolUpdate.selectSymbols model [ sym.Id ])
                |> set (sheet_ >-> SheetT.selectedComponents_) [ sym.Id ]
            else
                model
        model
    ...
    | DBWire(wire, aSeg), "Unfix Wire" ->
        let changeManualSegToAuto: BusWireT.Segment -> BusWireT.Segment =
            map BusWireT.mode_ (function
                | BusWireT.Manual -> BusWireT.Auto
                | m -> m)
        model
        |> map
            (sheet_
             >-> SheetT.wireOf_ wire.WId
             >-> BusWireT.segments_)
            (List.map changeManualSegToAuto)
    ...
    | DBCanvas pos, "Zoom-in (Alt+Up) and centre" ->
        printf "Zoom-in!!"
        model
        |> map (sheet_ >-> SheetT.zoom_) (fun zoom ->
            min Sheet.Constants.maxMagnification (zoom * Sheet.Constants.zoomIncrement))
...
let filterByOKSheets (model: Model) (sheet: string) =
    ...
    | Some p when
        p.LoadedComponents
        |> List.forall (fun ldc -> ldc.Name <> sheet)
        ->
        false
    | _ -> true
...
let readUserData (userAppDir: string) (model: Model) : Model * Cmd<Msg> =
    ...
    let modelOpt =
        try
            let jsonRes =
                tryReadFileSync
                <| pathJoin [| userAppDir; "IssieSettings.json" |]
            jsonRes
...
let verilogOutputPage sheet fPath =
    div
        []
        [ str
              $"You can write sheet '{sheet}' (and its subsheets) in either simulation or synthesis format. The output will be written to:"
          Text.div
              [ Modifiers [ Modifier.TextWeight TextWeight.Bold ]
                Props
                    [ Style
                          [ TextAlign TextAlignOptions.Center
                            CSSProp.Padding "10px"
                            FontFamily "monospace"
                            FontSize "15px" ] ] ]
              [ str $"%s{Helpers.cropToLength 55 false fPath}.v" ]
          Columns.columns
              []
              [ Column.column
                    []
                    [ Panel.panel
                          ...
                          [ Panel.heading [] [ str "Simulation output" ]
                            Panel.Block.div
                                []
                                [ str
                                      "Simulation output will run on an online synthesis tool such as Icarus v10 to check that Issie's Verilog output is working" ]
                            Panel.Block.div
                                []
                                [ Button.button
                                      [ Button.Color IsSuccess
                                        ...
                                        Button.IsFullWidth
                                        Button.OnClick
                                        <| openInBrowser "https://www.tutorialspoint.com/compile_verilog_online.php" ]
                            ...
                            Panel.Block.div
                                ...
                                [ Button.button
                                      [ Button.Color IsSuccess
                                        Button.IsFullWidth
                                        Button.OnClick
                                        <| openInBrowser "https://github.com/edstott/issie-synth" ]
...
let getMenuView (act: MenuCommand) (model: Model) (dispatch: Msg -> Unit) =
    match act with
    | MenuSaveFile ->
        MenuHelpers.saveOpenFileActionWithModelUpdate model dispatch
        |> ignore
        SetHasUnsavedChanges false
        |> JSDiagramMsg
        |> dispatch
    | MenuSaveProjectInNewFormat ->
        MenuHelpers.saveOpenProjectInNewFormat model
        |> ignore
    | MenuNewFile -> TopMenuView.addFileToProject model dispatch
    | MenuLostFocus -> printf "Lost focus!"

    | MenuExit -> TopMenuView.doActionWithSaveFileDialog "Exit ISSIE" CloseApp model dispatch ()
    | MenuVerilogOutput ->
...
let updateTimeStamp model =
    let setTimeStamp (lc: LoadedComponent) =
        { lc with TimeStamp = System.DateTime.Now }
    match model.CurrentProj with
    | None -> model
    | Some p ->
        p.LoadedComponents
        |> List.map (fun lc ->
            if lc.Name = p.OpenFileName then
                setTimeStamp lc
            else
                lc)
...
let findChange (model: Model) : bool =
    ...
    | Some prj ->
        ...
        (canv <> canv')
        && not (compareCanvas 100. canv canv')
...
let resetDialogIfSelectionHasChanged newModel oldModel : Model =
    ...
    if
        ...
        && newSelected <> oldModel.Sheet.SelectedComponents
    then
        newModel
        |> map popupDialogData_ (set text_ None >> set int_ None)
    else
        newModel
...
let executePendingMessagesF n model =
    if n = (List.length model.Pending) then
