...
let update (msg: Msg) (model: Model) : Model * Cmd<Msg> =
    ...
    | Wire(BusWireT.Symbol SymbolT.Msg.UpdateBoundingBoxes) ->
        ...
        model, Cmd.batch [ Cmd.ofMsg UpdateBoundingBoxes ]
    | Wire wMsg ->
    ...
    | KeyPress CtrlZ ->
        ...
        | prevModel :: lst ->
            let symModel =
                { prevModel.Wire.Symbol with CopiedSymbols = model.Wire.Symbol.CopiedSymbols }

            let wireModel =
                { prevModel.Wire with
                    CopiedWires = model.Wire.CopiedWires
                    Symbol = symModel }

            { prevModel with
                Wire = wireModel
                UndoList = lst
                RedoList = model :: model.RedoList
    ...
    | KeyPress CtrlW ->
        ...
        model', Cmd.batch [ Cmd.ofMsg (UpdateScrollPos paras.Scroll); Cmd.ofMsg UpdateBoundingBoxes ]
    ...
    | KeyPress ZoomOut ->
        ...
        { model with Zoom = newZoom }, Cmd.ofMsg (KeepZoomCentered oldScreenCentre)
    ...
    | CheckAutomaticScrolling ->
        ...
        if xDiff <> 0.0 || yDiff <> 0.0 then // Did any automatic scrolling happen?
            ...
            let filteredOutputCmd =
                Cmd.map (fun msg -> if notAutomaticScrolling msg then msg else DoNothing) outputCmd
    ...
    | ColourSelection(compIds, connIds, colour) ->
        { model with
            SelectedComponents = compIds
            SelectedWires = connIds },
        Cmd.batch
            ...
            Async.StartImmediate(
                async {
                    ...
                    try
                        ...
                        let c = simpleConnect ()
                        c.``then`` (fun v -> dispatch <| (DebugRead viewers)) |> ignore
    ...
    | OnDebugRead(data, whichViewer) ->
        ...
        { model with DebugData = List.insertAt part data (List.removeAt part model.DebugData) }, Cmd.none
    ...
    | TestPortPosition ->
        ...
        validateTwoSelectedSymbols model
        |> function
            | Some(s1, s2) -> { model with Wire = SmartSizeSymbol.reSizeSymbol model.Wire s1 s2 }, Cmd.none
            | None ->
                printfn "Error: can't validate the two symbols selected to reorder ports"
                model, Cmd.none
    | TestSmartChannel ->
