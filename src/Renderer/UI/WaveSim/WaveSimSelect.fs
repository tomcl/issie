...
let getInputPortName (compType: ComponentType) (port: InputPortNumber) : string =
    let muxPortName (size: int) : string =
        if port = (InputPortNumber size) then
            ".SEL"
        else
            "." + string port
    ...
    | Custom c -> "." + fst c.InputLabels[getInputPortNumber port]
...
let getInputName (withComp: bool) (comp: FastComponent) (port: InputPortNumber) : string =
    ...
    let bitLims: string =
        ...
        | CounterNoLoad _
        | CounterNoEnableLoad _
        | NbitSpreader _ -> bitLimsString (0, 0)
        ...
        | NbitsAdderNoCin w
        | NbitsAdderNoCout w
        | NbitsAdderNoCinCout w -> bitLimsString (w - 1, 0)
        ...
        | Custom c -> bitLimsString (snd c.InputLabels[getInputPortNumber port] - 1, 0)
...
let getOutputPortName (compType: ComponentType) (port: OutputPortNumber) : string =
    ...
    | NbitsAdderNoCin _ ->
        match port with
        | OutputPortNumber 0 -> ".SUM"
        | _ -> ".COUT"
    | NbitsAdderNoCout _
    | NbitsAdderNoCinCout _ -> ".SUM"
    ...
    | AsyncROM1 _
    | ROM1 _ -> ".DOUT"
    | Custom c -> "." + fst c.OutputLabels[getOutputPortNumber port]
...
let getOutputName
    ...
    let bitLims =
        ...
        | CounterNoEnable w
        | CounterNoLoad w
        | CounterNoEnableLoad w -> bitLimsString (w - 1, 0)

        | Shift(w, m, tp) -> bitLimsString (w - 1, 0)
        | RAM1 mem
        | AsyncRAM1 mem
        | AsyncROM1 mem
        | ROM1 mem -> bitLimsString (mem.WordWidth - 1, 0)

        | Custom c -> bitLimsString (snd c.OutputLabels[getOutputPortNumber port] - 1, 0)
        ...
        | IOLabel ->
            ...
            let labelWidth =
                FastRun.extractFastSimulationWidth
                    fastSim
                    (drivingComp.Id, snd drivingComp.fId)
                    (OutputPortNumber 0)
            //printfn "label width fdone"
            match labelWidth with
            | None -> failwithf $"What? Can't find width for IOLabel {comp.FLabel}$ "
            | Some width -> bitLimsString (width - 1, 0)
...
let caseCompAndPortName (name: string) =
    ...
    | 1 -> name.ToUpper()
    | n ->
        (String.concat "." parts[0 .. n - 2]).ToUpper()
        + "."
        + camelCaseDottedWords parts[n - 1]
...
let nameWithSheet (fastSim: FastSimulation) (dispName: string) (waveIndex: WaveIndexT) =
    ...
    | path ->
        camelCaseDottedWords (path[path.Length - 2])
        + "."
        + dispName
...
let makeWave (ws: WaveSimModel) (fastSim: FastSimulation) (wi: WaveIndexT) : Wave =
    ...
    let driver =
        ...
        match fastSim.Drivers[wi.SimArrayIndex - 1] with
        | Some d -> d
        | None ->
            printfn "What? No driver!"
            printfn
                $"ERROR Making wave for {fc.FullName}, portType={wi.PortType}, portNumber={wi.PortNumber}, SubSheet={fc.SubSheet}, SheetName={fc.SheetName}"
            printfn
                $"Can't find simulation waveform driver for {fc.FullName}.{wi.PortType}[{wi.PortNumber}]"
            failwithf "Aborting..."
...
let toggleSelectAll (selected: bool) (wsModel: WaveSimModel) dispatch : unit =
    ...
    dispatch
    <| GenerateWaveforms { wsModel with SelectedWaves = selectedWaves }
...
let selectAll (wsModel: WaveSimModel) dispatch =
    let allWavesSelected =
        Map.forall (fun index _ -> isWaveSelected wsModel index) wsModel.AllWaves

    tr
        (summaryProps false (SheetItem []) wsModel dispatch)
        [ th
              []
              [ Checkbox.checkbox
                    []
                    [ Checkbox.input
                          [ Props(
                                ...
                                @ [ Checked allWavesSelected
                                    OnChange(fun _ ->
                                        toggleSelectAll (not allWavesSelected) wsModel dispatch) ]
                            ) ] ] ]
          th [] [ str "Select All" ] ]
...
let checkboxRow (wsModel: WaveSimModel) dispatch (index: WaveIndexT) =
    let fontStyle =
        if isWaveSelected wsModel index then
            boldFontStyle
        else
            normalFontStyle
...
let infoButton: ReactElement =
    div
        [ HTMLAttr.ClassName
              $"{Tooltip.ClassName} {Tooltip.IsMultiline} {Tooltip.IsInfo} {Tooltip.IsTooltipRight}"
...
let searchBar (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    div
        ...
        [ Input.text
              [ Input.Option.Props
                    [ Style [ MarginBottom "1rem"; Width "30%"; Float FloatOptions.Left ] ]
                Input.Option.Placeholder "Search string"
                Input.Option.OnChange(fun c ->
                    dispatch
                    <| UpdateWSModel(fun ws -> { wsModel with SearchString = c.Value.ToUpper() })) ]
          infoButton
          label
              [ Style [ Float FloatOptions.Right; FontSize "24px" ] ]
              [ str $"{wsModel.SelectedWaves.Length} waves selected." ] ]
    ...
    Checkbox.checkbox
        ...
        [ Checkbox.input
              [ Props
                    [ Checked isChecked
                      OnChange(fun _ ->
                          toggleSelectSubGroup wsModel dispatch (not isChecked) waveIds) ] ] ]
        ...
        [ Checkbox.input
              [ Props
                    [ Checked checkBoxState
                      OnChange(fun _ ->
                          toggleSelectSubGroup wsModel dispatch (not checkBoxState) waveIds) ] ] ]
...
let rec makeComponentGroup
    ...
    (waves: Wave list)
    =
    let compWaves = waves |> List.groupBy (fun wave -> wave.WaveId.Id)
    if compWaves.Length = 1 then
        ...
        let compRows =
            compWaves
            |> List.map (fun (fId, compWaves) ->
                makeComponentRow showDetails ws dispatch ws.FastSim.WaveComps[fId] compWaves)

        makeSelectionGroup showDetails ws dispatch summaryReact compRows cBox waves

let rec makeSheetRow
    (showDetails: bool)
    (ws: WaveSimModel)
    (dispatch: Msg -> Unit)
    (subSheet: string list)
    ...
    let componentRows =
        ...
        |> List.map (fun (grp, groupWaves) ->
            makeComponentGroup showDetails ws dispatch subSheet grp groupWaves)

    let subSheetRows =
        wavesBySheet
        |> List.filter (fun (g, _) -> g <> subSheet)
        |> List.map (fun (subSheet', waves') ->
            makeSheetRow showDetails ws dispatch subSheet' waves')

    let rows = List.append subSheetRows componentRows
    if subSheet = [] then
        Table.table
            [ Table.IsBordered; Table.IsFullWidth; Table.Props [ Style [ BorderWidth 0 ] ] ]
            [ tbody [] rows ]
    else
        makeSelectionGroup
            showDetails
...
let selectWaves (ws: WaveSimModel) (subSheet: string list) (dispatch: Msg -> unit) : ReactElement =
    if not ws.WaveModalActive then
        ...
        let wavesToDisplay =
            ...
            | "-" when
                ws.ShowSheetDetail.Count <> 0
                || ws.ShowComponentDetail.Count <> 0
                || ws.ShowGroupDetail.Count <> 0
                ->
                dispatch
                <| SetWaveSheetSelectionOpen(ws.ShowSheetDetail |> Set.toList, false)
                dispatch
                <| SetWaveGroupSelectionOpen(ws.ShowGroupDetail |> Set.toList, false)
                dispatch
                <| SetWaveComponentSelectionOpen(ws.ShowComponentDetail |> Set.toList, false)
                []
            | ""
            | "-" -> waves
            | "*" ->
            ...
            | _ -> List.filter (fun x -> x.ViewerDisplayName.ToUpper().Contains(searchText)) waves
        let showDetails =
            ((wavesToDisplay.Length < 10)
             || searchText.Length > 0)
            && searchText <> "-"
        wavesToDisplay
...
let selectWavesButton (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    ...
    let props, buttonFunc =
        if waveCount > 0 && wsModel.State = Success then
            ...
            (fun _ ->
                dispatch
                <| UpdateWSModel(fun ws -> { wsModel with WaveModalActive = true }))
        else
            selectWavesButtonPropsLight, (fun _ -> ())
    button props buttonFunc (str "Select Waves")
...
let selectWavesModal (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    let endModal _ =
        dispatch
        <| UpdateWSModel(fun ws -> { wsModel with WaveModalActive = false; SearchString = "" })
    Modal.modal
        ...
        [ Modal.background
              [ Props
                    [ OnClick(fun _ ->
                          dispatch
                          <| UpdateWSModel(fun ws -> { wsModel with WaveModalActive = false })) ] ]
              []
          Modal.Card.card
              [ Props [ Style [ MinWidth "900px" ] ] ]
              [ Modal.Card.head
                    []
                    [ Modal.Card.title
                          []
                                ...
                                [ Level.left [] [ str "Select Waves" ]
                                  Level.right
                                      ...
                                      [ Delete.delete
                                            ...
                                            [] ] ] ] ]
                Modal.Card.body
                    [ Props [ Style [ OverflowY OverflowOptions.Visible ] ] ]
                    [ searchBar wsModel dispatch; selectWaves wsModel [] dispatch ]
                Modal.Card.foot
                    [ Props
                          [ Style [ Display DisplayOptions.InlineBlock; Float FloatOptions.Right ] ] ]
                    [ Button.button
...
let selectRamButton (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    ...
    let props, buttonFunc =
        if ramCount > 0 && wsModel.State = Success then
            selectRamButtonProps,
            (fun _ ->
                dispatch
                <| UpdateWSModel(fun ws -> { wsModel with RamModalActive = true }))
        else
            selectRamButtonPropsLight, (fun _ -> ())
    button props buttonFunc (str "Select RAM")
...
let toggleRamSelection (ramId: FComponentId) (ramLabel: string) (wsModel: WaveSimModel) dispatch =
    ...
    dispatch
    <| UpdateWSModel(fun ws -> { wsModel with SelectedRams = selectedRams })
    ...
    Modal.modal
        ...
        [ Modal.background
              [ Props
                    [ OnClick(fun _ ->
                          dispatch
                          <| UpdateWSModel(fun ws -> { wsModel with RamModalActive = false })) ] ]
              []
          Modal.Card.card
              []
              [ Modal.Card.head
                                            ...
                                            [ Delete.Option.Size IsMedium
                                              Delete.Option.OnClick(fun _ ->
                                                  ...
                                                  <| UpdateWSModel(fun ws ->
                                                      { wsModel with RamModalActive = false })) ]
                                            [] ] ] ] ]
                Modal.Card.body
                    []
                    [ str "Select synchronous RAM components to view their contents. "
                      str
                          "Note that asynchronous RAM components cannot be viewed in the waveform simulator. "
                      br []
                      br []
                      ...
                      str
                          "Any memory address which has not been initialised with a value will not be shown in the table. "
                      hr []
                      Table.table [] [ tbody [] (List.map (ramRows) wsModel.RamComps) ] ]
