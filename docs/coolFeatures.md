...
| **Edit a library** | Catalogue → `Library` → a library → `Edit` | The library opens as a project, each component a sheet — a multi-sheet component brings all of its sheets. Saving writes back into the library in place. Offered only for libraries you can save: not the ones shipped inside the installation |
