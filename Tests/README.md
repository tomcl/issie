...
```bash
CI=true npm run test          # 629 tests, ~20s
```
...
| `WireQuality.fs` | 10 | What "the wiring looks good" means as numbers - wire drawn, bends, crossings, cross-net overlap - over a corpus of the sheets that are hard to route, with the current scores recorded so a change to routing or separation has to argue for itself. |
...
```
  ...
  wrappedArrays     10     8104 ->   10966   49 ->  58     8 ->   36          0    4.1  at once
  fanout            36     9366 ->    9002   99 ->  98     0 ->    0       3257    4.8  at once
  staggeredFanout    8     3662 ->    3878   31 ->  34     8 ->    9       1515    1.1  at once
