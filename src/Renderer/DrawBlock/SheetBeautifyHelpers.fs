...
let getBlockAbstracted (symbols: Symbol list) : BoundingBox =
    ...
    let listMapXY total (f: 'a -> XYPos) (xyL: 'a list) =
        toXY (total (List.map (fun xy -> (f xy).X) xyL)) (total (List.map (fun xy -> (f xy).Y) xyL))

    // Calculate the maximum X & Y position of a symbol edge in the bounding box plus its rotated width
    let maxXY =
        symbols
        |> listMapXY List.max (fun sym -> sym.Pos + getWH sym)
...
let putCustomCompDims (newDims: XYPos) (sym: Symbol) =
    ...
    let ws', hs' = // HScale, VScale affect h,w before rotation, so we need to compensate for that
        ...
        | Degree0
        | Degree180 -> ws * newDims.X / w, hs * newDims.Y / h // non-rotated case
        | Degree90
        | Degree270 -> hs * newDims.Y / w, ws * newDims.X / h // rotated case: swap w,h
    { sym with HScale = Some ws'; VScale = Some hs' }
...
let updateSymPos (newPos: XYPos) (sym: Symbol) =
    { sym with Pos = newPos } |> calcLabelBoundingBox // recalculate the label bounding box in the symbol
...
let updateSymPosInSheet (symId: ComponentId) (newPos: XYPos) (sheet: SheetT.Model) =
    ...
    |> Optic.set
        (SheetT.boundingBoxes_
         >-> Optics.Map.valueForce_ "Bounding box not found" symId
         >-> topLeft_)
        newPos
...
let getPortOrder edge symbol = symbol.PortMaps.Order.[edge]
...
let reversedInputPorts_ =
    Lens.create (fun symbol -> symbol.ReversedInputPorts) (fun newState symbol ->
        { symbol with ReversedInputPorts = newState })
...
let symbol_rotation_ =
    Lens.create (fun symbol -> symbol.STransform.Rotation) (fun newState symbol ->
        { symbol with STransform = { symbol.STransform with Rotation = newState } })

//B8 RW
/// A lens for accessing the flip state of a symbol.
let symbol_flipped_ =
    Lens.create (fun sym -> sym.STransform.Flipped) (fun newState sym ->
        { sym with STransform = { sym.STransform with Flipped = newState } })
...
let segSameNetCheck (seg1: Segment) (seg2: Segment) (model: SheetT.Model) =
