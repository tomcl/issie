...
module Converter =
    ...
    let rec private _createTypeInfo (resolvedType: Type) : Fable.SimpleJson.TypeInfo =
        ...
        // ISSIE PATCH (Fable 5): lists and options must be recognised BEFORE the union check.
        // F# lists and options ARE union types, and Fable 5's .NET-faithful reflection reports
        // them as such; Fable 4 reported IsUnion = false for them, which is the only reason
        // upstream's union-first order worked. With the union check first, every list and
        // option takes the union path, which cannot serialise their erased runtime
        // representations (JS arrays / undefined) and cannot parse their JSON.
        ...
        // ISSIE PATCH (Fable 5): OptionType moved above UnionType, see comment there
