...
let newWire inputId outputId model =
    ...
    if
        Map.exists
            (fun wid wire ->
                wire.InputPort = nWire.InputPort
                && wire.OutputPort = nWire.OutputPort)
            model.Wires
    then
        // wire already exists
        model, None
...
let calculateBusWidths model =
    ...
    let processConWidths (connWidths: ConnectionsWidth) =
        let addWireWidthFolder (wireMap: Map<ConnectionId, Wire>) _ wire =
            let width =
                ...
                | None -> wire.Width
            let newColor =
                if wire.Color = Purple || wire.Color = Brown then
                    Purple
                else
                    DarkSlateGrey
        ...
        let addSymbolWidthFolder (m: Map<ComponentId, Symbol>) (_: ConnectionId) (wire: Wire) =
            let inPort =
                model.Symbol.Ports[match wire.InputPort with
                                   | InputPortId ip -> ip]
            ...
            | MergeWires ->
                match inPort.PortNumber with
                | Some 0 -> Map.add symId { symbol with InWidth0 = Some wire.Width } m
                | Some 1 -> Map.add symId { symbol with InWidth1 = Some wire.Width } m
            ...
            | MergeN nInps ->
                ...
                | Some n when (n < nInps && n >= 0) ->
                    let newInWidths: int option list =
                        ...
                        | None -> List.init nInps (fun i -> if i = n then Some wire.Width else None)
                        | Some list ->
                            match list.Length with
                            | len when len = nInps -> List.mapi (fun i x -> if i = n then Some wire.Width else x) list
                            | _ -> List.init nInps (fun i -> if i = n then Some wire.Width else None)
                    Map.add symId { symbol with InWidths = Some newInWidths } m
                | x ->
                    failwithf $"What? wire found with input port {x} other than [0..{nInps - 1}] connecting to MergeN"
            | _ -> m

        let newWires =
            ((Map.empty, model.Wires)
             ||> Map.fold addWireWidthFolder)

        let symbolsWithWidths =
            (model.Symbol.Symbols, newWires)
            ||> Map.fold addSymbolWidthFolder
    ...
    let canvasState =
        (SymbolUpdate.extractComponents model.Symbol, extractConnections model)

    match BusWidthInferer.inferConnectionsWidth canvasState with
    | Ok connWidths -> processConWidths connWidths, None
    | Error e -> { model with Notifications = Some e.Msg }, Some(ErrorWires e.ConnectionsAffected)

/// Handles messages
let update (msg: Msg) (issieModel: ModelType.Model) : ModelType.Model * Cmd<ModelType.Msg> =
    let model = issieModel.Sheet.Wire

    let toIssieModel wireModel =
        { issieModel with Sheet = { issieModel.Sheet with Wire = wireModel } }
    ...
    let withSheetMsg (msg: DrawModelType.SheetT.Msg) (model: ModelType.Model) =
        model, Cmd.ofMsg (ModelType.Msg.Sheet msg)
    ...
    let withOptMsg (msgOpt: Msg option) =
        (if msgOpt.IsSome then
             withMsg (Option.get msgOpt)
         else
             withNoMsg)
    ...
    let withMsgs (msgs: Msg list) (model: Model) =
        let wireMsg msg =
            Cmd.ofMsg (ModelType.Msg.Sheet(DrawModelType.SheetT.Msg.Wire msg))
    ...
    | Symbol sMsg ->
        ...
        { issieModel with
            Sheet = { issieModel.Sheet with Wire = { model with Symbol = sm } } },
        (Cmd.map (fun msg -> ModelType.Msg.Sheet(DrawModelType.SheetT.Msg.Wire msg)) sCmd)
    ...
    | UpdateWires(componentIdList, diff) ->
        ...
        { issieModel with
            Sheet = { issieModel.Sheet with Wire = updateWires model componentIdList diff } }
        |> withNoMsg
        ...
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = BusWireSeparate.routeAndSeparateSymbolWires model compId } }
        |> withNoMsg
    ...
    | AddWire((inputId, outputId): (InputPortId * OutputPortId)) ->
        ...
        { issieModel with Sheet = { issieModel.Sheet with Wire = newModel } }
        |> (if msgOpt.IsSome then
                withMsg (Option.get msgOpt)
            else
                withNoMsg)
    ...
    | BusWidths ->
        ...
        toIssieModel newModel |> withOptMsg msgOpt

    | CopyWires(connIds: list<ConnectionId>) ->
        // add given wires to Copiedwires state (NB, this contains wires at time of copy)
        let copiedWires =
            Map.filter (fun connId _ -> List.contains connId connIds) model.Wires
        { issieModel with
            Sheet = { issieModel.Sheet with Wire = { model with CopiedWires = copiedWires } } }
        |> withNoMsg
    ...
    | ErrorWires(connectionIds: list<ConnectionId>) ->
        ...
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = { model with Wires = newWires; ErrorWires = connectionIds } } }
        |> withNoMsg
    ...
    | SelectWires(connectionIds: list<ConnectionId>) ->
        ...
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = { model with Wires = newWires; ErrorWires = [] } } }
        |> withNoMsg
    ...
    | DeleteWires(connectionIds: list<ConnectionId>) ->
        ...
        let model = { model with Wires = newWires }
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = { model with Wires = newWires; ErrorWires = connectionIds } } }
    ...
    | DragSegment(segIdL: SegmentId list, mMsg: MouseT) ->
        ...
        |> function
            ...
            | segIdL ->
                ...
                | Down ->
                    { issieModel with
                        Sheet = { issieModel.Sheet with Wire = { model with SelectedSegment = segIdL } } }
                    |> withMsg (ResetJumps [])
                | Drag ->
                    (model, segIdL)
        ...
        let newWires =
            (List.fold
                (fun prevWires cId ->
                    ...
                    | None -> prevWires
                    | Some oldWire -> Map.add cId { oldWire with Color = color } prevWires)
                model.Wires
                connIds)
        { issieModel with
            Sheet = { issieModel.Sheet with Wire = { model with Wires = newWires } } }
        |> withNoMsg
    ...
    | ResetModel ->
        ...
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = { model with Wires = Map.empty; ErrorWires = []; Notifications = None } } }
        |> withNoMsg
    ...
    | ToggleArrowDisplay ->
        { issieModel with
            Sheet =
                { issieModel.Sheet with
                    Wire = { model with ArrowDisplay = not model.ArrowDisplay } } }
        |> withNoMsg
    ...
    | UpdateConnectedWires(componentIds: ComponentId list) ->
        ...
        let updatePortIdMessages: seq<Cmd<ModelType.Msg>> =
            ...
            |> (fun (m1, m2) ->
                let inputPorts =
                    Seq.map (fun (InputPortId portId) -> portId) m1.Keys
                    |> Seq.toList
                let outputPorts =
                    Seq.map (fun (OutputPortId portId) -> portId) m2.Keys
                    |> Seq.toList
    ...
    | RerouteWire(portId: string) ->
        ...
        let rerouteInputEnd (wire: Wire) = wire.InputPort = InputPortId portId

        let wiresToReroute =
            model.Wires
            |> Map.filter (fun _id wire ->
                wire.InputPort = InputPortId portId
                || wire.OutputPort = OutputPortId portId)
            |> Map.toList
    ...
    | ToggleSnapToNet ->
        { issieModel with
            Sheet = { issieModel.Sheet with Wire = { model with SnapToNet = not model.SnapToNet } } }
        |> withNoMsg
...
let getClickedWire (wModel: Model) (pos: XYPos) (n: float) : ConnectionId Option =
    let boundingBox =
        { BoundingBox.TopLeft = { X = pos.X - n; Y = pos.Y - n }
          H = n * 2.
          W = n * 2. }
    let intersectingWires = getIntersectingWires (wModel: Model) boundingBox
    List.tryHead intersectingWires |> Option.map fst
...
let pasteWires (wModel: Model) (newCompIds: list<ComponentId>) : (Model * list<ConnectionId>) =
        ...
        let createNewWire (oldWire: Wire) : list<Wire> =
            ...
            | Some(newInputPort, newOutputPort) ->
                ...
                let outputPortOrientation =
                    getOutputPortOrientation wModel.Symbol (OutputPortId newOutputPort)
                let segmentList =
    ...
    let newWireMap =
        Map.fold (fun acc newKey newVal -> Map.add newKey newVal acc) pastedWires wModel.Wires
    let pastedConnIds = pastedWires |> Map.toList |> List.map fst
