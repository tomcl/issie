...
let view
    ...
    let dragToSelectBox =
        ...
        let selectionBoxDev = {Stroke = "Black"; StrokeWidth = "0.1px"; Fill = "Red"; FillOpacity = 0.05 }

        /// Makes a polygon ReactElement for a developer mode dragToSelectBox, with added text info
        /// points are to be given as a correctly formatted SVGAttr.Points string
        let makePolygonDevMode (points: string) (polygonParameters: Polygon) (debugOutput: string) =
            let debugOutputLength = debugOutput.Length
            let firstPoint = points.Split ' ' |> Array.head |> fun s -> s.Split ','
            let textX, textY = firstPoint.[0], firstPoint.[1]
            g [] [
            polygon [
            ...
            rect [
                SVGAttr.X textX
                SVGAttr.Y (string(float textY - 14.))
                SVGAttr.Width (string (debugOutput.Length * 6))
                SVGAttr.Height "14"
                SVGAttr.Fill "white"
            ] []
            text [
                SVGAttr.X (string(float textX + 5.))
                SVGAttr.Y  textY
                SVGAttr.FontSize "10"
                SVGAttr.Dy "-3"
            ] [ str debugOutput ]
            ]
        if debugLevel > 0 && model.DeveloperModeTabActive then
            let fw_shortened = fW.ToString("F1")
            let fh_shortened = fH.ToString("F1")
            let debugOutput = $"{fw_shortened} x {fh_shortened}"
            makePolygonDevMode polygonPoints selectionBoxDev debugOutput
        else
