...
/// Read one or more signals at one cycle, for the SYNCHRONOUS path.
///
/// The same question `read` answers, in the shape a synchronous caller can ask it: no framing, no
/// windows, no repetition - a cycle and a handful of signals, answered as text. That is the whole
/// of what synchronous is for. Anything bulk stays on the asynchronous binary path, because the
/// renderer's synchronous transport (a blocking XMLHttpRequest) cannot receive binary at all.
///
/// **Called from a different connection to the one `serve` is on**, which is safe because of an
/// invariant the RENDERER keeps: a synchronous operation is issued only when nothing is in flight
/// (`Model.SidecarInFlight` is empty), and two synchronous operations cannot overlap because one
/// runs to completion on the renderer's single thread. So this never runs beside a build or a run.
/// It is not defended against here, because a defence would be a mechanism this protocol has
/// nowhere else - see the module note on SimInterface.
let readAt
    (askedEpoch: int)
    (cycle: int)
    (signals: (ComponentId * ComponentId list * int) list)
    : Result<bigint list, string> =
    match checkEpoch askedEpoch, session with
    | Error _, _ ->
        Error $"stale session: command names epoch {askedEpoch}, this sidecar holds {currentEpoch ()}"
    | _, None -> Error "no simulation built - send SimBuild first"
    | Ok(), Some(fs, _) ->
        if List.isEmpty signals then
            Error "no signals asked for"
        elif fs.ClockTick < cycle then
            Error $"simulation has only reached cycle {fs.ClockTick}"
        elif fs.ClockTick - cycle >= fs.MaxArraySize then
            Error $"cycle {cycle} is outside the {fs.MaxArraySize}-entry circular buffer"
        else
            // Caught rather than allowed to escape: a signal this simulation does not hold is an
            // ordinary thing for a caller to ask about - a probe on a wire of a sheet that is
            // instantiated more than once, a selection made before a rebuild - and the answer to
            // it is "no", not an exception crossing a process boundary as a failed HTTP request.
            try
                signals
                |> List.map (fun (cid, path, outPort) ->
                    match FastExtract.extractFastSimulationOutput fs cycle (cid, path) (OutputPortNumber outPort) with
                    | SimGraphTypes.IData fd -> fd.GetBigInt
                    | SimGraphTypes.IAlg _ -> 0I)
                |> Ok
            with e ->
                Error $"this simulation cannot read one of those signals: {e.Message}"
