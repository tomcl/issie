...
let tests =
    testList "Carrier" [
        test "a design-enumerated selection survives the slice gap and then resolves" {
            ...
            Expect.equal (kept |> List.filter (fun w -> dToInt w.SimArrayIndex >= 0) |> List.length) 0
            ...
            Expect.equal (resolved |> List.filter (fun w -> dToInt w.SimArrayIndex >= 0) |> List.length) 100
