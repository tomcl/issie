...
type XYPos =
    ...
    static member inline (=~)(left: XYPos, right: XYPos) =
        abs (left.X - right.X) <= XYPos.epsilon
        && abs (left.Y - right.Y) <= XYPos.epsilon
...
let private testXYPosComparison a (b: XYPos) = a =~ b
...
type ComponentType =
    ...
    | NbitsAdderNoCin of BusWidth: int
    | NbitsAdderNoCout of BusWidth: int
    | NbitsAdderNoCinCout of BusWidth: int
    | NbitsXor of BusWidth: int * ArithmeticOp: NBitsArithmetic option
    | NbitsAnd of BusWidth: int
    | NbitsNot of BusWidth: int
    | NbitsOr of BusWidth: int
    | NbitSpreader of BusWidth: int
    | Custom of CustomComponentType // schematic sheet used as component
    | MergeWires
    | SplitWire of BusWidth: int // int is bus width
    ...
    | DFF
    | DFFE
    | Register of BusWidth: int
    | RegisterE of BusWidth: int
    | Counter of BusWidth: int
    | CounterNoLoad of BusWidth: int
    | CounterNoEnable of BusWidth: int
    | CounterNoEnableLoad of BusWidth: int
    | AsyncROM1 of Memory1
    | ROM1 of Memory1
    | RAM1 of Memory1
    | AsyncRAM1 of Memory1
...
let portOrder_ =
    Lens.create (fun c -> c.PortOrder) (fun n c -> { c with PortOrder = n })
let portOrientation_ =
    Lens.create (fun c -> c.PortOrientation) (fun n c -> { c with PortOrientation = n })
...
let inputPorts_ =
    Lens.create (fun c -> c.InputPorts) (fun n c -> { c with InputPorts = n })
let outputPorts_ =
    Lens.create (fun c -> c.OutputPorts) (fun n c -> { c with OutputPorts = n })
...
module LegacyCanvas =
    ...
    /// JSConnection mapped to F# record.
    /// Id uniquely identifies connection globally and is used by library.
    type LegacyConnection = { Id: string; Source: Port; Target: Port; Vertices: (float * float) list }
...
type WaveIndexT = { SimArrayIndex: int; Id: FComponentId; PortType: PortType; PortNumber: int }
...
type SheetInfo = { Form: CCForm option; Description: string option }
...
let formOpt_ =
    Lens.create (fun a -> a.Form) (fun s a ->
        match s with
        | None -> a
        | Some s -> { a with Form = Some s })
let canvasState_ =
    Lens.create (fun a -> a.CanvasState) (fun s a -> { a with CanvasState = s })
...
let loadedComponents_ =
    Lens.create (fun a -> a.LoadedComponents) (fun s a -> { a with LoadedComponents = s })

let openLoadedComponent_ =
    Lens.create (fun a -> List.find (fun lc -> lc.Name = a.OpenFileName) a.LoadedComponents) (fun lc' a ->
        { a with
            LoadedComponents =
                List.map
                    (fun lc ->
                        if lc.Name = a.OpenFileName then
                            lc'
                        else
                            lc)
                    a.LoadedComponents })
...
let openFileName_ =
    Lens.create (fun a -> a.OpenFileName) (fun s a -> { a with OpenFileName = s })

let loadedComponentOf_ (name: string) =
    Lens.create (fun a -> List.find (fun lc -> lc.Name = name) a.LoadedComponents) (fun lc' a ->
        { a with
            LoadedComponents = List.map (fun lc -> if lc.Name = name then lc' else lc) a.LoadedComponents })
