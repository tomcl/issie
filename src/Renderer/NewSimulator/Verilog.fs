...
let verilogNameConvert (maxChars: int) (s: string) =
    ...
    let baseName =
        ...
        |> function
            ...
            | [] -> "x"
        |> Seq.map (function
            | ch when System.Char.IsLetterOrDigit ch || ch = '_' -> string ch
            | _ -> "")
...
let writeVerilogNames (fs: FastSimulation) =
    ...
    let namesWithFC =
        ...
        |> Array.map (fun (fid, fc) -> getBaseVerilogName fc, fc)
    ...
    let rec disambiguate names : (string * FastComponent) array =
        if Array.length (Array.distinctBy fst names) = names.Length then
            names
        else
            names
            |> Array.groupBy fst
...
let getVerilogComponent (fs: FastSimulation) (fc: FastComponent) =
    ...
    let name = fc.VerilogComponentName
    let idNum = name |> String.split [| '_' |] |> Array.last

    let outW i =
        match fc.OutputWidth[i] with
        | Some n when n > 64 ->
            failwithf "Sorry - Verilog output does not yet work for busses > 64 bit. Output failed"
        | Some n -> n
    ...
    let inW i =
        ...
        |> function
            | Some n -> n
            | None ->
                failwithf "Can't find output width for output port %d of %A\n" opn fs.FComps[fid]

    let demuxOutput (outputPort: string) (selectPort: string) (w: int) =
        if outputPort = selectPort then
            ins 0
        else
            makeBits w (uint64 0)
    ...
    | Counter _ ->
        $"always @(posedge clk) %s{outs 0} <= %s{ins 2} ? (%s{ins 1} ? %s{ins 0} : (%s{outs 0}+1'b1)) : %s{outs 0};\n"
    | CounterNoEnable _ ->
        $"always @(posedge clk) %s{outs 0} <= %s{ins 1} ? %s{ins 0} : (%s{outs 0}+1'b1) ;\n"
    | CounterNoLoad _ ->
        $"always @(posedge clk) %s{outs 0} <= %s{ins 0} ? (%s{outs 0}+1'b1) : %s{outs 0};\n"
    | CounterNoEnableLoad _ -> $"always @(posedge clk) %s{outs 0} <= (%s{outs 0}+1'b1) ;\n"
    | DFF
    | Register _ -> $"always @(posedge clk) %s{outs 0} <= %s{ins 0};\n"
    | Constant1(w, c, _)
    | Constant(w, c) -> $"assign %s{outs 0} = %s{makeBits w (uint64 c)};\n"
    | Decode4 ->
        let w = outW 1
    ...
    | NbitSpreader n ->
        let a = ins 0
        let out = outs 0
        let result1 = ("", [ 1..n ]) ||> List.fold (fun s v -> s + "1")
        // $"assign {out} = {a} << {n});\n"
        $"assign {out} = {a} ? {n}'b{result1} : {n}'b0;\n"
    | Mux2 -> $"assign %s{outs 0} = %s{ins 2} ? %s{ins 1} : %s{ins 0};\n"
    | Mux4 ->
        $"assign %s{outs 0} = %s{ins 4}[1] ? (%s{ins 4}[0] ? %s{ins 3} : %s{ins 2}) : (%s{ins 4}[0] ? %s{ins 1} : %s{ins 0})  ;\n"
    ...
    | BusCompare1(w, c, _) ->
        $"assign %s{outs 0} = %s{ins 0} == %s{makeBits w (uint64 (uint32 c))};\n"
    ...
    | RAM1 mem
    | AsyncRAM1 mem -> $"%s{name} I{idNum} (%s{outs 0}, %s{ins 0}, %s{ins 1}, %s{ins 2}, clk);\n"
    | Custom _ -> failwithf "What? custom components cannot exist in fast Simulation data structure"
    | Input _
    | AsyncROM _
    | RAM _
    | ROM _ -> failwithf $"Invalid legacy component type '{fc.FType}'"
    | Shift(n, m, tp) ->
        let input = ins 0
        let shifter = ins 1
        let output = outs 0
...
let getMainHeader (vType: VMode) (profile: CompilationProfile) (fs: FastSimulation) =
    ...
    |> Array.collect (fun fc -> // NB - inputs are assigned zero and not included in module header
        match fc.FType, fc.AccessPath with
    ...
    |> Array.append (
        ...
        | ForSynthesis ->
            match profile with
            | Release -> [| "clk" |]
            | Debug -> [| "debug_clk"; "RS232_Rx_TTL"; "RS232_Tx_TTL" |]
        | ForSimulation -> [||]
    )
    |> String.concat ",\n\t"
    |> (fun header ->
    ...
    let outNames, (outFormat, outVars) =
        ...
        |> Array.map (fun (_, fc) ->
            ...
            let hexWidth =
                ...
                if w <= 0 then
                    failwithf $"Unexpected width ({w})in verilog output for {fc.FullName}"
                (w - 1) / 4 + 1
