...
let tryBitwiseOperation (expressions: FastAlgExp list) =
    ...
    | (BinaryExp(UnaryExp(BitRangeOp(_, _), left), bop, UnaryExp(BitRangeOp(_, _), right))) :: tl ->
        let rec checkList exps state remBits =
            ...
            | (BinaryExp(UnaryExp(BitRangeOp(ll, lu), l), op, UnaryExp(BitRangeOp(rl, ru), r))) :: tl,
              true ->
                if
                    ll = lu
                    && ll = rl
        ...
        let dat =
            if outW <= 32 then
                if bits < 0I || bits >= (1I <<< 32) then
                    printf
                        $"""HELP! weird bits = {bits.ToString("X")} mask = {mask} msb,lsb = ({msb},{lsb})"""
...
type FData =
    ...
    member this.toInt64 =
        match this with
        | Data { Width = w } when w > 64 ->
            failwithf "Help! Can't convert numbers wider than 64 bits to int64."
...
type FastComponent =
    ...
    member inline this.PutOutputFData (epoch) (OutputPortNumber n) dat =
        this.OutputsFData[n].Step[ epoch ] <- dat
    member inline this.PutOutput (epoch) (OutputPortNumber n) dat =
        this.Outputs[n].Step[ epoch ] <- dat
...
type FastSimulation =
    {
        ...
        FCustomOutputCompLookup:
            Map<(ComponentId * ComponentId list) * OutputPortNumber, FComponentId>
    ...
    | Custom _ ->
        ...
        let customOutput =
            ...
            |> function
                ...
                | None ->
                    failwithf
                        "Cannot find custom component %A in fast simulation"
                        (customFId, driverPort)
#if ASSERTS
        Helpers.assertThat
