...
let minEditorWidth () =
    int (
        (document.getElementById "WholeApp").offsetWidth
        * 0.25
    )
...
let rightSectionWidth (model: Model) =
    ...
    | RightTab.Properties
    | RightTab.Catalogue
    | RightTab.DeveloperMode
    | RightTab.Transition -> rightSectionWidthS
    | RightTab.Build -> rightSectionWidthL
    | RightTab.Simulation ->
        match model.SimSubTabVisible with
        | SimSubTab.StepSim -> rightSectionWidthL
        | SimSubTab.WaveSim
        | SimSubTab.TruthTable -> sprintf "%dpx" model.WaveSimViewerWidth

let leftSectionWidth model =
    Style [ Width(sprintf "calc(100%s - %s - 10px)" "%" (rightSectionWidth model)) ]

let navbarStyle model =
...
let belowHeaderStyle headerSize =
    Style [ OverflowY OverflowOptions.Auto; Height $"calc(100%% - {headerSize})" ]
...
let simulationNumberStyle = Style [ Width "320px"; Height "30px" ]

let constraintNumberStyle = Style [ Width "200px"; Height "30px" ]

let simulationBitStyle = Style [ Width "100px"; Height "30px"; PaddingTop "3px" ]
...
let ttGridContainerStyle model =
    let widthRightSec = rightSectionWidth model
    Style [ Display DisplayOptions.Grid; GridAutoFlow "column" ]
