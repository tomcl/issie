...
let beautify (model: SheetT.Model) (lengthThreshold: float) : SheetT.Model =
    replaceLongWiresWithLabels model lengthThreshold

let UserThresholdPopUp (model: ModelType.Model) (dispatch: Dispatch<Msg>) =
    ...
    let buttonAction =
        fun (model': Model) ->
            ...
            let newSheet = beautify sheet (float outputInt)
...
let beautifySheet (model: ModelType.Model) (dispatch: Dispatch<Msg>) : unit =
    ...
    UserThresholdPopUp model dispatch

let testsToRunFromSheetMenu: (string * (int -> int -> Dispatch<Msg> -> Unit)) list =
    // Change names and test functions as required
    // delete unused tests from list
    [ "Test1",
      fun _ _ _ -> printf "Test1" // dummy test - delete line or replace by real test as needed
      "Test2",
      fun _ _ _ -> printf "Test2" // dummy test - delete line or replace by real test as needed
      "Test3",
      fun _ _ _ -> printf "Test3" // dummy test - delete line or replace by real test as needed
      "Test4",
      fun _ _ _ -> printf "Test4" // dummy test - delete line or replace by real test as needed
      "Test5",
