...
let getUserAppDir () : string =
    unbox
    <| renderer.ipcRenderer.sendSync ("get-user-data", None)
...
let makeElmItem (label: string) (accelerator: string) (action: unit -> unit) =
    jsOptions<MenuItemConstructorOptions>
    <| fun item ->
        item.label <- Some label
...
let fileMenu (dispatch) =
    ...
    makeMenu
        ...
        [ makeItem "New Sheet" (Some "CmdOrCtrl+N") (fun ev -> dispatch (MenuAction(MenuNewFile, dispatch)))
          ...
          makeItem "Save Project in New Format" None (fun ev ->
              dispatch (MenuAction(MenuSaveProjectInNewFormat, dispatch)))
          ...
          makeDebugItem "Show Model Before Beautify" (Some "CmdOrCtrl+L") (fun ev ->
              dispatch
              <| Msg.ExecFuncInMessage(TestDrawBlock.HLPTick3.Tests.showUnitTestOnSheetWrapper, dispatch))
          makeDebugItem "Beautify Model" (Some "CmdOrCtrl+T") (fun ev ->
              dispatch
              <| Msg.ExecFuncInMessage(TestDrawBlock.HLPTick3.Tests.runUnitTestOnSheetWrapper, dispatch))
          makeDebugItem "Beautify Model" (Some "CmdOrCtrl+P") (fun ev ->
              dispatch
              <| Msg.ExecFuncInMessage(TestDrawBlock.HLPTick3.Tests.printTestModel, dispatch))
          ...
          makeMenuGen
              ...
              [ makeDebugItem "Set Scroll" None (fun _ -> SheetDisplay.writeCanvasScroll { X = 1000.; Y = 1000. })
                ...
                makeDebugItem "Trace short, medium & long times" None (fun _ ->
                    TimeHelpers.instrumentation <- TimeHelpers.ImmediatePrint(1.5, 1.5)
                    if debugTraceUI = Set.ofList [] then
                        debugTraceUI <- Set.ofList [ "update"; "view" ])
                makeDebugItem "Trace medium & long times" None (fun _ ->
                    TimeHelpers.instrumentation <- TimeHelpers.ImmediatePrint(3., 3.)
                    if debugTraceUI = Set.ofList [] then
                        debugTraceUI <- Set.ofList [ "update"; "view" ])
                makeDebugItem "Trace long times" None (fun _ ->
                    TimeHelpers.instrumentation <- TimeHelpers.ImmediatePrint(20., 20.)
                    if debugTraceUI = Set.ofList [] then
                        debugTraceUI <- Set.ofList [ "update"; "view" ])
                makeDebugItem "Highlight debugChangedConnections" None (fun _ ->
                    Playground.Misc.highLightChangedConnections dispatch)
                ...
                makeWinDebugItem "Test Breadcrumbs" None (fun _ ->
                    dispatch
                    <| Msg.ExecFuncInMessage(Playground.Breadcrumbs.testBreadcrumbs, dispatch))
                makeWinDebugItem "Test All Hierarchies Breadcrumbs" None (fun _ ->
                    dispatch
                    <| Msg.ExecFuncInMessage(Playground.Breadcrumbs.testAllHierarchiesBreadcrumbs, dispatch))

                makeDebugItem "Force Exception" None (fun ev -> failwithf "User exception from menus")

                makeDebugItem "Web worker performance test" None (fun _ ->
                    Playground.WebWorker.testWorkers Playground.WebWorker.Constants.workerTestConfig)
...
let viewMenu dispatch =
    ...
    let symbolDispatch msg =
        busWireDispatch (BusWireT.Msg.Symbol msg)
    ...
    makeMenu
        ...
        [ makeRoleItem "Toggle Fullscreen" (Some "F11") MenuItemRole.Togglefullscreen
          ...
          makeCondItem (JSHelpers.debugLevel <> 0) "Toggle Dev Tools" (Some devToolsKey) (fun _ ->
              renderer.ipcRenderer.send ("toggle-dev-tools", [||])
              |> ignore) ]
    ...
    <| fun invisibleMenu ->
        ...
        invisibleMenu.submenu <-
            [| // makeElmItem "Save Sheet" "CmdOrCtrl+S" (fun () -> ())
               ...
               makeElmItem "Flip Vertically" "CmdOrCtrl+Up" (fun () ->
                   sheetDispatch <| SheetT.Flip SymbolT.FlipVertical)
               makeElmItem "Flip Horizontally" "CmdOrCtrl+Down" (fun () ->
                   sheetDispatch
               ...
               menuSeparator
               makeElmItem "Align" "CmdOrCtrl+Shift+A" (fun ev ->
                   sheetDispatch
                   <| SheetT.Arrangement SheetT.AlignSymbols)
               makeElmItem "Distribute" "CmdOrCtrl+Shift+D" (fun ev ->
                   sheetDispatch
                   <| SheetT.Arrangement SheetT.DistributeSymbols)
               makeElmItem "Rotate Label Clockwise" "CmdOrCtrl+Shift+Right" (fun ev ->
                   sheetDispatch <| SheetT.RotateLabels)
               menuSeparator
               makeElmItem "Select All" "CmdOrCtrl+A" (fun () -> dispatch SheetT.KeyboardMsg.CtrlA)
               makeElmItem "Delete" (if isMac then "Backspace" else "delete") (fun () ->
                   dispatch SheetT.KeyboardMsg.DEL)
...
let attachMenusAndKeyShortcuts dispatch =
    ...
    let sub dispatch =
        ...
        dispatch
        <| Msg.ExecFuncInMessage((fun _ _ -> electronRemote.app.applicationMenu <- Some menu), dispatch)
...
let keyPressListener initial =
    let subDown dispatch =
        Browser.Dom.document.addEventListener (
            ...
            fun e ->
                ...
                if
                    (jsToBool ke.ctrlKey || jsToBool ke.metaKey)
                    && firstPress
                then
                    firstPress <- false
    ...
    let subRightClick dispatch =
        Browser.Dom.document.addEventListener (
            "contextmenu",
            unbox (fun (e: Browser.Types.MouseEvent) ->
                e.preventDefault ()
    ...
    let subContextMenuCommand dispatch =
        renderer.ipcRenderer.on (
            ...
            fun ev args ->
                ...
                | _ -> printfn "Unexpected callback argument sent from main."
        )
        |> ignore
