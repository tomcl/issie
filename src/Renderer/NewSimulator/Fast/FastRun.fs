...
let printComp (fs: FastSimulation) (step: int) (fc: FastComponent) =
    let attr =
        [ if isComb fc then "Co" else "  "
          ...
          "    "
          (fc.InputLinks
           |> Array.map (fun (arr: StepArray<FastData>) ->
               arr.Step.Length > 0 && isValidData arr.Step[step])
           |> Array.map (function
               | true -> "*"
               | false -> "X")
           |> String.concat "") ]
...
let printCompFData (fs: FastSimulation) (step: int) (fc: FastComponent) =
    let attr =
        [ if isComb fc then "Co" else "  "
          ...
          "    "
          (fc.InputLinksFData
           |> Array.map (fun (arr: StepArray<FData>) ->
               arr.Step.Length > 0
               && isValidDataFData arr.Step[step])
           |> Array.map (function
               | true -> "*"
               | false -> "X")
           |> String.concat "") ]
...
let private orderCombinationalComponents (numSteps: int) (fs: FastSimulation) : FastSimulation =
    ...
    let initInput (fc: FastComponent) =
        ...
        |> Array.iteri (fun i _ ->
            fc.InputLinks[0].Step[ i ] <-
                convertIntToFastData (Option.defaultValue 1 fc.OutputWidth[0]) 0u)
    ...
    let initClockedOuts (fc: FastComponent) =
        ...
        |> Array.iteri (fun i vec ->
            ...
            | AsyncRAM1 _, _ ->
                failwithf
                    "What? Bad initial values for RAM %s output %d state <%A>"
                    fc.FullName
                    i
                    fc.FType
...
let private setSimulationInputFData
    (cid: ComponentId)
    (fd: FData)
    (step: int)
    (fs: FastSimulation)
...
let extractStatefulComponents (step: int) (fastSim: FastSimulation) =
    ...
    |> Array.collect (fun fc ->
        ...
        | [] ->
            ...
            | _ ->
                failwithf
                    "Unsupported state extraction from clocked component type %s %A"
                    fc.FullName
                    fc.FType
        | _ -> [||])
...
let extractViewers (simulationData: SimulationData) : ((string * string) * int * FSInterface) list =
    ...
    |> List.map (fun (fid, fc) ->
        ...
        getFLabel fs fid,
        width,
        extractFastSimulationOutput fs simulationData.ClockTickNumber fid (OutputPortNumber 0))

let compareLoadedStates (fs: FastSimulation) (canv: CanvasState) (p: Project option) =
    List.forall
        (fun ldc -> Extractor.loadedComponentIsSameAsProject canv ldc p)
