...
let productn (s: seq<#seq<'a>>) =
    s
    |> Seq.fold (fun r s -> r |> product s) (seq { yield Seq.empty })
...
let constrainedValuesandLength { Equalities = equ; Inequalities = ineq } =
    let equValues = equ |> Seq.map (fun con -> con.Value)
...
let tableLHS
    ...
    let inputValuesSeq count (io: SimulationIO) =
        ...
        | { Equalities = []; Inequalities = [] } -> Seq.init seqLength id, count * seqLength
        | conSet ->
    ...
    let numericVals, constrainedRowCount =
        (1, numericInputs) ||> List.mapFold inputValuesSeq
...
let simulateInputCombination
    ...
    |> List.map (fun cell ->
        ...
        | SimIO(cid, _, _), Bits wd -> (cid, IData(convertWireDataToFastData wd))
...
let truthTable
    ...
    let tableSimData =
        if isRegeneration then
            ...
            | Ok fs -> { simData with FastSim = fs }
            | _ -> failwithf "Error in building fast simulation for Truth Table evaluation"
    let inputs =
        List.map fst (FastRun.extractFastSimulationIOsFData simData.Inputs tableSimData)
    let outputs =
        List.map fst (FastRun.extractFastSimulationIOsFData simData.Outputs tableSimData)
