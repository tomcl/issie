...
let displayValuesOnWave
    wsModel
    (waveValues: FastData array)
    ...
    let gaps: Gap array =
        ...
        |> Array.map (fun (i1, i2) -> { Start = i1; Length = i2 - i1 })
    gaps
    // Create text react elements for each gap
    |> Array.map (fun gap ->
        let waveValue =
            fastDataToPaddedString
                WaveSimHelpers.Constants.waveLegendMaxChars
                wsModel.Radix
                waveValues[gap.Start]

        /// Amount of whitespace between two Change transitions minus the crosshatch
        let cycleWidth = singleWaveWidth wsModel
        let availableWidth =
            (float gap.Length * cycleWidth)
            - 2. * Constants.nonBinaryTransLen
        /// Required width to display one value
        let requiredWidth =
            1.1
            * DrawHelpers.getTextWidthInPixels Constants.valueOnWaveText waveValue
        ...
        if availableWidth < requiredWidth then
            ...
            let repeatSpace =
                (availableWidth - float repeats * requiredWidth)
                / ((float repeats + 1.) * cycleWidth)
            let valueText i =
                text
                    (valueOnWaveProps wsModel i (float gap.Start + repeatSpace) widthWithPadding)
                    [ str waveValue ]

            [ 0 .. repeats - 1 ] |> List.map valueText)
    |> List.concat
...
let private setClkCycle (wsModel: WaveSimModel) (dispatch: Msg -> unit) (newClkCycle: int) : unit =
    ...
    if newClkCycle <= endCycle wsModel then
        if newClkCycle < wsModel.StartCycle then
            ...
            dispatch
            <| SetWSModel { wsModel with CurrClkCycle = newClkCycle; ClkCycleBoxIsEmpty = false }
    else
        dispatch
        <| GenerateWaveforms
            { wsModel with
...
let changeZoom (wsModel: WaveSimModel) (zoomIn: bool) (dispatch: Msg -> unit) =
    ...
    let shownCycles =
        let wantedCycles =
            int (
                float wsModel.ShownCycles
                / Constants.zoomChangeFactor
            )
        if zoomIn then
            ...
            |> (fun nc ->
                if nc = wsModel.ShownCycles then
                    nc - 1
                else
                    nc)
            ...
            |> (fun nc ->
                let maxNc =
                    int (
                        wsModel.WaveformColumnWidth
                        / float Constants.minCycleWidth
                    )
    ...
    let startCycle =
        ...
        let sc =
            (wsModel.StartCycle
             - (shownCycles - wsModel.ShownCycles) / 2)
    ...
    dispatch
    <| GenerateWaveforms { wsModel with ShownCycles = shownCycles; StartCycle = startCycle }
...
let zoomButtons (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    div
        [ clkCycleButtonStyle ]
        [ button
              [ Button.Props [ clkCycleLeftStyle ] ]
              (fun _ -> changeZoom wsModel false dispatch)
              zoomOutSVG
          button
              [ Button.Props [ clkCycleRightStyle ] ]
...
let clkCycleButtons (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    ...
    div
        ...
        [
          ...
          button [ Button.Props [ clkCycleInnerStyle ] ] (fun _ -> scrollWaveformsBy -1) (str "◀")
          ...
          Input.number
              [ Input.Props clkCycleInputProps
                ...
                Input.OnChange(fun c ->
                    ...
                    | true, n -> setClkCycle wsModel dispatch n
                    | false, _ when c.Value = "" ->
                        dispatch
                        <| SetWSModel { wsModel with ClkCycleBoxIsEmpty = true }
                    | _ ->
                        dispatch
                        <| SetWSModel { wsModel with ClkCycleBoxIsEmpty = false }) ]
...
let private radixButtons (wsModel: WaveSimModel) (dispatch: Msg -> unit) : ReactElement =
    let radixString = [ Bin, "Bin"; Hex, "Hex"; Dec, "uDec"; SDec, "sDec" ]

    let radixTab (radix, radixStr) =
        Tabs.tab
            [ Tabs.Tab.IsActive(wsModel.Radix = radix); Tabs.Tab.Props radixTabProps ]
            [ a
                  [ radixTabAStyle
                    OnClick(fun _ ->
                        dispatch
                        <| GenerateWaveforms { wsModel with Radix = radix }) ]
                  [ str radixStr ] ]

    Tabs.tabs [ Tabs.IsToggle; Tabs.Props [ radixTabsStyle ] ] (List.map (radixTab) radixString)

let highlightCircuit fs comps wave (dispatch: Msg -> Unit) =
    dispatch
    <| Sheet(SheetT.Msg.Wire(BusWireT.Msg.Symbol(SymbolT.SelectSymbols comps)))
                  ...
                  [ nameRowLevelStyle (wsModel.HoveredLabel = Some wave.WaveId)
                    ...
                    OnMouseOver(fun _ ->
                        if wsModel.DraggedIndex = None then
                            dispatch
                            <| SetWSModel { wsModel with HoveredLabel = Some wave.WaveId }
                            ...
                            | Some { Component = { Type = IOLabel; Label = lab } } ->
                                let labelComps =
                                    ...
                                    |> List.filter (function
                                        | { Type = IOLabel; Label = lab' } when lab' = lab -> true
                                        | _ -> false)
                    ...
                    OnMouseOut(fun _ ->
                        dispatch
                        <| SetWSModel { wsModel with HoveredLabel = None }
                        dispatch
                        <| Sheet(SheetT.Msg.Wire(BusWireT.Msg.Symbol(SymbolT.SelectSymbols [])))
                        dispatch
                        <| Sheet(
                            SheetT.Msg.UpdateSelectedWires(connsOfWave wsModel.FastSim wave, false)
                        ))
                    ...
                    OnDrag(fun ev ->
                        ...
                        if
                            ev.clientX < bcr.left
                            || ev.clientX > bcr.right
                            || ev.clientY < bcr.top
                            || ev.clientY > bcr.bottom
                        then
                            dispatch
                            <| SetWSModel
                                { wsModel with
                    ...
                    OnDragEnter(fun ev ->
                        ...
                        let bcr = nameColEl.getBoundingClientRect ()
                        let index =
                            int (ev.clientY - bcr.top) / Constants.rowHeight
                            - 1
                        let draggedWave =
                        ...
                        dispatch
                        <| SetWSModel { wsModel with SelectedWaves = selectedWaves })

                    OnDragEnd(fun _ ->
                        dispatch
                        <| SetWSModel { wsModel with DraggedIndex = None; PrevSelectedWaves = None }) ] ]
            [ Level.left
                  ...
                  [ Delete.delete
                        [ Delete.Option.Size IsSmall
                          Delete.Option.Props
                              [ OnClick(fun _ ->
                                    let selectedWaves =
                                        List.except [ wave.WaveId ] wsModel.SelectedWaves
                                    dispatch
                                    <| SetWSModel { wsModel with SelectedWaves = selectedWaves }) ] ]
                        [] ]
              Level.right
...
let namesColumn model wsModel dispatch : ReactElement =
    ...
    div (namesColumnProps wsModel) (List.concat [ topRow; rows ])
...
let valueRows (wsModel: WaveSimModel) =
    let valueColWidth, valueColNumChars = valuesColumnSize wsModel
    selectedWaves wsModel
...
let private valuesColumn wsModel : ReactElement =
    ...
    div [ valuesColumnStyle width ] (List.concat [ topRow; rows ])
...
let clkCycleNumberRow (wsModel: WaveSimModel) =
    let makeClkCycleLabel i =
        ...
        | width when
            width < Constants.clkCycleNarrowThreshold
            && i % 5 <> 0
            ->
            []
...
let waveformColumn (wsModel: WaveSimModel) dispatch : ReactElement =
    ...
    let waveRows: ReactElement list =
        ...
        |> List.map (fun wave ->
            ...
            | None -> div [] [] // the GenerateCurrentWaveforms message will soon update this
        )
    ...
    div
        ...
        [ clkCycleHighlightSVG wsModel dispatch
          div
              [ waveRowsStyle wsModel.WaveformColumnWidth ]
              ([ clkCycleNumberRow wsModel ] @ waveRows) ]
...
let ramTableRow ((addr, data, rowType): string * string * RamRowType) : ReactElement =
    ...
    tr [ Style <| ramTableRowStyle rowType ] [ td [] [ str addr ]; td [] [ str data ] ]
...
let ramTable (wsModel: WaveSimModel) ((ramId, ramLabel): FComponentId * string) : ReactElement =
    ...
    let print w (a: int64) =
        NumberHelpers.valToPaddedString w wsModel.Radix (((1L <<< w) - 1L) &&& a)

    let lastLocation = int64 ((2 <<< memData.AddressWidth - 1) - 1)

    /// print a single 0 location as one table row
    let print1 (a: int64, b: int64, rw: RamRowType) =
        $"{print aWidth a}", $"{print dWidth b}", rw
    /// print a range of zero locations as one table row

    let print2 (a1: int64) (a2: int64) (d: int64) =
        $"{print aWidth (a1 + 1L)}..{print aWidth (a2 - 1L)}", $"{print dWidth d}", RAMNormal
    ...
    let printGap (gStart: int64) (gEnd: int64) =
        ...
        | n when n > 2L -> [ print2 gStart gEnd 0L ]
        | _ ->
    ...
    let addGapLines (items: (int64 * int64 * RamRowType) list) =
        ...
        |> List.collect (fun ((gStart, _, _), (gEnd, dEnd, rwe)) ->
            let thisItem =
                if gEnd = lastLocation + 1L then
                    []
                else
                    [ print1 (gEnd, dEnd, rwe) ]
    ...
    let addReadWrite (fc: FastComponent) (step: int) (mem: Map<int64, int64>) =
        ...
        let readOpt =
            ...
            | 0, ROM1 _
            | 0, RAM1 _ -> None
            | _ -> addrSteps readStep |> getFastData |> Some
        let writeOpt =
            match step, fc.FType with
            | _, ROM1 _
            | _, AsyncROM1 _
            | 0, _ -> None
            | _, RAM1 _
            | _, AsyncRAM1 _ when getFastData fc.InputLinks[2].Step[step - 1] = 1L ->
                addrSteps (step - 1) |> Some
            | _ -> None
    ...
    let addEndPoints (items: (int64 * int64 * RamRowType) list) =
        ...
        | _ ->
            if ad items[0] < 0L then
                items
            else
                List.insertAt 0 (-1L, -1L, RAMNormal) items
            |> (fun items ->
    ...
    Level.item
        ...
        [ Heading.h6 [ Heading.Option.Props [ centerAlignStyle ] ] [ str ramLabel ]
          div
              [ Style [ MaxHeight "600px"; OverflowY OverflowOptions.Auto ] ]
              [ Table.table
                                  ...
                                  th
                                      ...
                                      [ str "Data"
                                        sub
                                            ...
                                            [ str (string wsModel.CurrClkCycle) ] ] ] ]
                      tbody [] (List.map ramTableRow lineItems) ] ]
          br [] ]

/// Bulma Level component of tables showing RAM contents.
let ramTables (wsModel: WaveSimModel) : ReactElement =
    let inlineStyle (styles: CSSProp list) =
        div [ Style(Display DisplayOptions.Inline :: styles) ]
    let start = TimeHelpers.getTimeMs ()
    let selectedRams = Map.toList wsModel.SelectedRams
    if List.length selectedRams > 0 then
        let headerRow =
            [ "read", RAMRead; "overwritten", RAMWritten ]
            |> List.map (fun (op, opStyle) ->
                inlineStyle [] [ inlineStyle (ramTableRowStyle opStyle) [ str op ] ])
            |> function
                | [ a; b ] ->
                    [ str "Key: Memory location is "
                      a
                      str ", or "
                      b
                      str ". Click waveforms or use cursor control to change current cycle." ]
                | _ -> failwithf "What? Can't happen!"
        List.map
            (fun ram -> td [ Style [ BorderColor "white" ] ] [ ramTable wsModel ram ])
            selectedRams
        |> (fun tables ->
            [ tbody
                  []
                  [ tr [] [ th [ ColSpan selectedRams.Length ] [ inlineStyle [] headerRow ] ]
                    tr [ Style [ Border "10px" ] ] tables ] ])
        |> Fulma.Table.table
            [ Table.TableOption.Props ramTablesLevelProps
              Table.IsFullWidth
              Table.IsBordered
              Table.Props [ Style [ Height "100%" ] ] ]
    else
        div [] []
...
let makeWaveformsWithTimeOut
    ...
    let allWaves, numberDone, timeToDo =
        ...
        ||> List.fold (fun (all, n, _) wi ->
            ...
            | _ -> (Map.change wi (Option.map (generateWaveform ws wi)) all), n + 1, None)
...
let updateSpinner (name: string) payload (numToDo: int) (model: Model) =
    ...
    | Some sp when sp.Name = name ->
        { model with
            SpinnerPayload =
                Some { Name = name; Payload = payload; ToDo = numToDo; Total = sp.Total } }
    | _ ->
        { model with
            SpinnerPayload =
                Some { Name = name; Payload = payload; ToDo = numToDo; Total = numToDo + 1 } }

/// remove the spinner popup
let cancelSpinner (model: Model) = { model with SpinnerPayload = None }
...
let rec refreshWaveSim
    (newSimulation: bool)
    (wsModel: WaveSimModel)
    (model: Model)
    : Model * Elmish.Cmd<Msg>
    =
    let isSameWave (wi: WaveIndexT) (wi': WaveIndexT) =
        wi.Id = wi'.Id
        && wi.PortNumber = wi'.PortNumber
        && wi.PortType = wi'.PortType
    ...
    if fs.NumStepArrays = 0 then
        ...
        |> (fun speedOpt ->
            ...
            match speedOpt with
            | Some speed when
                float cyclesToDo / speed
                + Constants.initSimulationTime > Constants.maxSimulationTimeWithoutSpinner
            ...
            | _ ->
                if speedOpt <> None then
                    ...
                    FastRun.runFastSimulation None lastCycleNeeded fs
                    |> ignore
                ...
                let simulationIsUptodate =
                    wsModel.FastSim.ClockTick > wsModel.ShownCycles + wsModel.StartCycle
                ...
                let wavesToBeMade =
                    ...
                    |> Map.filter (fun wi wave ->
                        ...
                        List.exists (fun wi' -> isSameWave wi wi') wsModel.SelectedWaves
                        && hasChanged
                        && simulationIsUptodate)
                    |> Map.toList
                    |> List.map fst
                ...
                let model, allWaves, spinnerPayload, numToDo =
                    ...
                    makeWaveformsWithTimeOut
                        (Some Constants.initSimulationTime)
                        wsModel
                        allWaves
                        wavesToBeMade
                    |> (fun (allWaves, numDone, timeOpt) ->
                        match wavesToBeMade.Length - numDone, timeOpt with
                        | n, None -> model, allWaves, None, n // finished
                        | _ when numDone = 0 ->
                            failwithf
                                "What? makewaveformsWithTimeOut must make at least one waveform"
                        | numToDo, Some t when
                            float wavesToBeMade.Length * t / float numDone < Constants.maxSimulationTimeWithoutSpinner
                            ->
                            let (allWaves, numDone, timeOpt) =
                                makeWaveformsWithTimeOut None wsModel allWaves wavesToBeMade
                            model, allWaves, None, numToDo - numDone
                        | numToDo, _ ->
                            let payload =
                                Some(
                                    "Making waves",
                                    refreshWaveSim false { wsModel with AllWaves = allWaves }
                                    >> fst
                                )
                ...
                let selectedWaves =
                    wsModel.SelectedWaves
                    |> List.collect (fun wi ->
                        match Array.tryFind (isSameWave wi) allWaveA with
                        | Some w -> [ w ]
                        | None -> [])

                let selectedRams =
                    Map.filter
                        (fun ramfId _ -> List.contains ramfId ramCompIds)
                ...
                let model =
                    ...
                    | Some sp -> updateSpinner (fst sp) (snd sp) numToDo model
...
let refreshButtonAction canvasState model dispatch =
    fun _ ->
        ...
        match
            SimulationView.simulateModel
                model.WaveSimSheet
                (WaveSimHelpers.Constants.maxLastClk
                 + WaveSimHelpers.Constants.maxStepsOverflow)
                canvasState
                model
        with
        //| None ->
        ...
        | (Error e, _) ->
            dispatch
            <| SetWSModelAndSheet({ wsModel with State = SimError e }, wsSheet)
...
let topHalf canvasState (model: Model) dispatch : ReactElement =
    ...
    let loading =
        ...
        | Loading -> true
        | _ -> false
    let refreshButtonSvg =
        if loading then
            emptyRefreshSVG
        else
            refreshSvg "white" "20px"
    ...
    div
        ...
        [ Columns.columns
              []
              [ Column.column
                    [ Column.Props [ Style [ Height "200px"; OverflowY OverflowOptions.Clip ] ] ]
                    [ Heading.h4
                          []
                          [ (div
                                [ Style [ Display DisplayOptions.Inline; MarginRight "10px" ] ]
                                [ str title ])
                            button
                      ...
                      div
                          ...
                          (if model.WaveSimSheet <> None then
                               [ str
                                     "Use 'Select Waves' to select and add clocked logic waveforms. "
                                 str
                                     "Use 'Select RAM' to view RAM or ROM contents during the simulation. "
                    ...
                    [ let startOrRenew = refreshButtonAction canvasState model dispatch
                      ...
                      div
                          ...
                          (if not wbo.IsRunning then
                               ...
                               [ startEndButton
                                 button
                                     (Button.Disabled(not needsRefresh)
                      ...
                      Level.level
                          []
                          [ Level.left
                                [ GenericOption.Props [ Style [ MarginLeft "5px" ] ] ]
                                [ zoomButtons wsModel dispatch ]
                            Level.right [] [ radixButtons wsModel dispatch ] ]
...
let viewWaveSim canvasState (model: Model) dispatch : ReactElement =
    ...
    let simError e =
        ...
        div [ errorMessageStyle ] [ SimulationView.viewSimulationError e ]
    div
        []
        [ div
              [ viewWaveSimStyle ]
              [ topHalf canvasState model dispatch
                ...
                | Some sheet as sheetOpt, SimError e when sheetOpt <> getCurrFile model ->
                    dispatch
                    <| UpdateModel(fun model -> { model with WaveSimSheet = None })
                    dispatch
                    <| UpdateModel(updateWSModelOfSheet sheet (fun ws -> { ws with State = Ended }))
                    notRunning
                | None, SimError e -> notRunning
                | _, SimError e -> simError e
                | _, NonSequential ->
                    div
                        [ errorMessageStyle ]
                        [ str
                              "There is no clocked logic in this circuit. Add clocked logic to simulate waveforms." ]
                | _, Empty
                | _, Ended
                | None, _
                | Some "", _ -> notRunning
                ...
                | _, Success ->
                    ...
                    div
                        ...
                        [ showWaveforms model wsModel dispatch; hr []; ramTables wsModel ]
