...
let verilogNameConvert (maxChars: int) (s: string) =
    ...
    let baseName =
        ...
        |> function
            ...
            | [] -> "x"
        |> Seq.map (function
            | ch when Char.IsLetterOrDigitOrUnderscore ch -> string ch
            | _ -> "")
...
let getVerilogComponent (fs: FastSimulation) (fc: FastComponent) =
    ...
    let inW i =
        ...
        fs.FComps[fid].OutputWidths[opn]
        |> function
            | 0 -> failwithf "Can't find output width for output port %d of %A\n" opn fs.FComps[fid]
            | n -> n
...
let getDebugController (profile: CompilationProfile) (fs: FastSimulation) =
    ...
    let comps =
        ...
        |> Array.filter (fun fc ->
            ...
            | Viewer _ -> true
            | _ -> false)
        |> Array.map (fun fc -> getVPortOut fc (OutputPortNumber 0), fc.OutputWidths[0])
