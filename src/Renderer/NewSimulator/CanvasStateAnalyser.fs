...
let private checkPortTypesAreConsistent (canvasState: CanvasState) : SimulationError option =
    let rec checkComponentPorts (ports: Port list) (correctType: PortType) =
        ...
        | port :: _ when port.PortType <> correctType ->
            Some
                { Msg =
                    sprintf
                        "%A port %d appears to be an %A port"
                        correctType
                        (Option.get port.PortNumber)
                        port.PortType
                  InDependency = None
                  ComponentsAffected = [ ComponentId port.HostId ]
                  ConnectionsAffected = [] }
    ...
    let rec checkComponentsPorts (components: Component list) =
        ...
        | comp :: components' ->
            match
                checkComponentPorts comp.InputPorts PortType.Input,
                checkComponentPorts comp.OutputPorts PortType.Output
            with
            | Some err, _
            | _, Some err -> Some err
...
let private countPortsConnections
    ...
    let rec countPortsConnections'
        (conns: Connection list)
        (counts: Map<'b, int * Connection list>)
        =
        match conns with
        | [] ->
            counts
...
let private checkIOLabels (canvasState: CanvasState) : SimulationError option =
        ...
        | comp :: comps' ->
            ...
            | Some compId ->
                Some
                    { Msg =
                        sprintf
                            "Two %s components cannot have the same label: %s."
                            ioType
                            comp.Label
                      InDependency = None
    ...
    match
        checkDuplicate inputs (toMap inputs) "Input",
        checkDuplicate outputs (toMap outputs) "Output"
    with
    | Some err, _
    | _, Some err -> Some err
    | None, None -> None
...
let checkCustomComponentForOkIOs
    (c: Component)
    ...
    |> Option.map (fun sheet ->
        sheet,
        compare sheet.InputLabels args.InputLabels,
        compare sheet.OutputLabels args.OutputLabels)
    |> function
...
let checkCustomComponentsOk
    ((comps, _): CanvasState)
    (sheets: LoadedComponent list)
    : SimulationError option
    =
    let error (c: Component) msg =
        Some
            { Msg = msg
              InDependency = None
    ...
    |> function
        ...
        | Error(c, NoSheet cName) ->
            error c
            <| sprintf
                "Can't find a design sheet named %s for the custom component of this name"
                cName
...
let analyseState (state: CanvasState) (ldComps: LoadedComponent list) : SimulationError option =
